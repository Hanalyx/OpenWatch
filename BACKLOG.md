...
| Item | Priority | Status | Notes |
|------|----------|--------|-------|
| **OS Discovery + OS Intelligence + `/activity` unified page** | P1 | In Progress (Phase 1.1 active) | Three-tier initiative: (1) **OS Discovery** — SSH fingerprint of os_family / os_version / kernel / architecture on first contact, denormalized onto `hosts` row + full `host_system_info` record. Independent of Kensa (Kensa auto-detects OS internally for scans; this is the operator-facing inventory layer). (2) **OS Intelligence** — recurring snapshot collection (packages / services / users / network / firewall / routes / metrics) + diff-driven append-only event log (`host_intelligence_events`). (3) **`/activity` page** — unified role-filtered feed of intelligence events + transactions + alerts + audit. **PR sequence (9 PRs across 3 phases + a Phase 4)**: 1.1 Discovery backend (spec + migration 0017 + `internal/intelligence/{probe,discovery}` + `POST /api/v1/hosts/{id}/discovery:run` + EventKindHostDiscovered + auto-trigger). 1.2 Discovery UI wiring. 2.1 Intelligence schema + collector. 2.2 Intelligence read API. 2.3 UI: Server intelligence card + Packages tab. 2.4 UI: Services + Users tabs. 2.5 UI: Network + Firewall + Hardware. 3.1 Differ + events table + 10 event codes. 3.2 Events API + SSE + Recent Activity card. 4.x: alerts persistence + UNION-ALL /activity endpoint + page. Inventory + scoping in conversation 2026-06-01. Design context in `app/docs/activity_and_os_intelligence.md` + `docs/openwatchos/04-SERVER-INTELLIGENCE.md`. |
