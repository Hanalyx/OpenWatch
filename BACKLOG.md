...
| Item | Priority | Status | Notes |
|------|----------|--------|-------|
| **OS Intelligence + `/activity` unified page** | P2 | Deferred (context captured) | Host-side security/account/system event collection (account locked, password expired, failed logins, service crashes, etc.) plus a single role-filtered `/activity` page that unifies OS Intelligence events, transactions, alerts, and audit. Full design — schemas, event taxonomy (~25 codes), pull-via-SSH collection, RBAC model, UNION-ALL pagination, 9-item PR sequence (~4-5 backend days) — captured in `app/docs/activity_and_os_intelligence.md`. Resume after frontend GUI direction is settled. |
