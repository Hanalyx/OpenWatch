...
| Item | Priority | Status | Notes |
|------|----------|--------|-------|
| **OS Discovery + OS Intelligence + `/activity` unified page** | P1 | Backend complete (PRs #440-#446); Frontend wiring active (PRs 5a/5b/5c/6/7/8) | Three-tier initiative covering OS Discovery, OS Intelligence, and unified /activity feed. **Backend status:** All seven backend PRs merged (2026-06-01): #440 Discovery, #441 Collector, #442 Intelligence API, #443 Scheduler, #444 alert-router persistence, #445 alert lifecycle + REST, #446 unified /activity UNION feed. Schema + endpoints + events all production. **Frontend wiring (NEW, post-2026-06-01):** PR 5a backend: add `os_family`/`os_version` to `HostResponse` schema + handler (~80 LOC). PR 5b frontend plumbing: regenerate openapi-typescript schema + extend `useLiveEvents.ts` topics for `host.os_discovered` + `host.intelligence_event` (~50 LOC). PR 5c host list: swap `detectOS()` hostname heuristic for real `os_family` from API in `HostsListPage.tsx:70-86, 1412` (~60 LOC). PR 6 host detail OS panel: wire `GET /api/v1/intelligence/state/{host_id}` into the System card (`HostDetailPage.tsx:1182-1219`) + page-head OS/Kernel slots (`:411-413`) + "Re-run discovery" button → `POST /api/v1/hosts/{id}/discovery:run`; design contract `docs/prototypes/openwatch-v1/Host Detail.html:1812-1866` (Operating system, Hardware, Network spec groups + Server intelligence count tiles at `:1747-1787`). PR 7 host detail Intelligence events feed: new panel rendering `GET /api/v1/intelligence/events?host_id=…`; landing site TBD per prototype review (third-column or sibling of CardRecentActivity); optionally light up the Packages/Services/Users/Network tabs from the snapshot. PR 8 settings scheduler: new `GET/PUT /api/v1/system/intelligence/config` backend (spec + endpoints) mirroring `ScanningPage.tsx:216-282` connectivity-config pattern. **Design source:** `app/docs/prototypes/openwatch-v1/` (Host Detail.html, Host Management.html, Settings.html). **Design context:** `app/docs/activity_and_os_intelligence.md`. |
