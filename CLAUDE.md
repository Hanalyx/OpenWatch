...
#### OWCAService (Compliance Intelligence)
**Location**: `backend/app/services/owca/`

**Purpose**: Unified platform for all compliance scoring, risk assessment, and intelligence operations.

**Architecture**: OWCA (OpenWatch Compliance Algorithm) is a multi-layered intelligence platform with 5 specialized layers:

```python
# CORRECT - Use OWCA service factory
from backend.app.services.owca import get_owca_service

owca = get_owca_service(db)  # Returns configured OWCAService instance

# Access Layer 0: Extraction (XCCDF parsing, severity risk)
xccdf_result = owca.extract_xccdf_score("/app/data/results/scan_123.xml")
severity_risk = owca.calculate_severity_risk(critical=5, high=10, medium=20)

# Access Layer 1: Core (compliance scoring)
score = owca.calculate_compliance_score(host_id="uuid", db=db)

# Access Layer 2: Framework (NIST, CIS, STIG intelligence)
nist_controls = owca.get_nist_controls(host_id="uuid")

# Access Layer 3: Aggregation (fleet statistics)
fleet_stats = owca.get_fleet_statistics(group_id="uuid")

# Access Layer 4: Intelligence (trends, predictions)
risk_forecast = owca.predict_risk_trend(host_id="uuid", days=30)
```

**5-Layer Architecture**:

**Layer 0: Extraction** (Added 2025-11-22)
- **Purpose**: Extract data from SCAP scan results
- **Location**: `backend/app/services/owca/extraction/`
- **Components**:
  - `XCCDFParser`: Extract native XCCDF scores from XML files
  - `SeverityCalculator`: Calculate NIST SP 800-30 risk scores
  - Security: XXE prevention, path traversal validation, 10MB file limit
- **Methods**:
  - `owca.extract_xccdf_score(result_file, user_id)` - Parse XCCDF XML (cached 5min)
  - `owca.calculate_severity_risk(critical, high, medium, low, info)` - Risk scoring
- **Example**:
```python
# Extract XCCDF score with caching
result = owca.extract_xccdf_score(
    result_file="/app/data/results/scan_123_xccdf.xml",
    user_id=current_user.id  # Optional: for audit logs
)
if result.found:
    print(f"Score: {result.xccdf_score}/{result.xccdf_score_max}")

# Calculate severity-weighted risk
risk = owca.calculate_severity_risk(
    critical=3,   # 3 * 10 = 30
    high=10,      # 10 * 5 = 50
    medium=25,    # 25 * 2 = 50
    low=50,       # 50 * 0.5 = 25
    user_id=current_user.id,  # Optional: for audit logs
    scan_id=scan.id           # Optional: for audit logs
)
print(f"Risk: {risk.risk_score} ({risk.risk_level})")
# Output: Risk: 155.0 (critical)
```

**Layer 1: Core**
- **Purpose**: Basic compliance score calculations
- **Location**: `backend/app/services/owca/core/`
- **Components**: `ComplianceScoreCalculator`
- **Methods**: `calculate_compliance_score()`, `get_compliance_tier()`
- **Example**:
```python
# Per-host compliance score
score = owca.calculate_compliance_score(host_id="uuid", db=db)
# Returns: ComplianceScore(score=87.5, tier="GOOD", passed=87, failed=13)
```

**Layer 2: Framework**
- **Purpose**: Framework-specific intelligence (NIST, CIS, STIG)
- **Location**: `backend/app/services/owca/framework/`
- **Components**: `NISTMapper`, `CISMapper`, `STIGMapper`
- **Methods**: `get_nist_controls()`, `get_cis_benchmarks()`, `get_stig_requirements()`

**Layer 3: Aggregation**
- **Purpose**: Fleet-wide statistics and rollups
- **Location**: `backend/app/services/owca/aggregation/`
- **Components**: `FleetAggregator`
- **Methods**: `get_fleet_statistics()`, `get_group_rollup()`

**Layer 4: Intelligence**
- **Purpose**: Trends, forecasting, predictive analytics
- **Location**: `backend/app/services/owca/intelligence/`
- **Components**: `TrendAnalyzer`, `RiskPredictor`
- **Methods**: `predict_risk_trend()`, `detect_anomalies()`

**Why Use OWCA Service**:
- **Single Source of Truth**: All compliance intelligence in one place
- **Automatic Caching**: Redis caching for expensive operations (5-minute TTL)
- **Audit Logging**: Built-in audit trail for all operations
- **Type Safety**: Pydantic models for all inputs/outputs
- **Consistent API**: Unified interface across all layers

**Security Controls**:
- **XXE Prevention**: lxml secure parser (resolve_entities=False, no_network=True)
- **Path Traversal**: Validates all file paths (rejects ../, validates against /app/data/)
- **File Size Limits**: 10MB maximum for XCCDF parsing (DoS prevention)
- **Input Validation**: Pydantic models validate all parameters
- **Audit Logging**: Comprehensive logging to openwatch.audit logger

**Performance Optimization**:
- **Redis Caching**: 5-minute TTL for XCCDF parsing results
- **Lazy Loading**: Frameworks loaded only when needed
- **Connection Pooling**: Database connections reused

**Migration from /scoring Module**:

The `/scoring` module was deprecated and migrated to OWCA Extraction Layer (Layer 0) on 2025-11-22.

```python
# OLD (Deprecated - issues DeprecationWarning)
from backend.app.services.scoring import (
    XCCDFScoreExtractor,
    SeverityWeightingService,
)
extractor = XCCDFScoreExtractor()
result = extractor.extract_native_score(file)

# NEW (Current - use OWCA)
from backend.app.services.owca import get_owca_service
owca = get_owca_service(db)
result = owca.extract_xccdf_score(file)  # Includes caching
```

**Class Renamings**:
- `XCCDFScoreExtractor` → `XCCDFParser` (more descriptive)
- `SeverityWeightingService` → `SeverityCalculator` (consistent naming)
- `RiskScoreResult` → `SeverityRiskResult` (avoid confusion with Intelligence layer)

**Documentation**:
- **Migration Guide**: `docs/OWCA_EXTRACTION_LAYER_MIGRATION.md`
- **Architecture**: See Layer descriptions above
- **Examples**: See code examples in each layer section

**When NOT to use OWCA Service**:
- Simple database queries (use QueryBuilder or SQLAlchemy ORM directly)
- Non-compliance-related operations
- When you only need a single extraction operation without caching
