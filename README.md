...
OpenWatch is open core. The OpenWatch core is licensed under the **Apache
License 2.0** (see [LICENSE](LICENSE) and [NOTICE](NOTICE)).

- Free to use, modify, self-host, and redistribute under Apache 2.0
- The compiled binary statically links the Kensa engine, which is BSL-1.1; a
  binary distribution is a combined Apache/BSL work (see [NOTICE](NOTICE))
- Optional enterprise feature modules under [`ee/`](ee/) are separately,
  commercially licensed (see [`ee/LICENSE`](ee/LICENSE)) and are not required to
  run OpenWatch

Third-party dependency licenses: [THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md).
Commercial / EE licensing: [legal@hanalyx.com](mailto:legal@hanalyx.com)
