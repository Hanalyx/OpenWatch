...
def upgrade() -> None:
    ...
    # Create enum types
    op.execute("CREATE TYPE user_roles AS ENUM ('super_admin', 'security_admin', 'security_analyst', 'compliance_officer', 'auditor', 'guest')")
    ...
    op.create_table('users',
        ...
        sa.Column('role', postgresql.ENUM('super_admin', 'security_admin', 'security_analyst', 'compliance_officer', 'auditor', 'guest', name='user_roles'), nullable=False),
    ...
    op.create_table('scans',
        ...
        sa.ForeignKeyConstraint(['content_id'], ['scap_content.id'], ),
    ...
    op.create_table('system_credentials',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('name', sa.String(length=100), nullable=False),
        sa.Column('description', sa.Text(), nullable=True),
    ...
    op.create_index(op.f('ix_system_credentials_ssh_key_fingerprint'), 'system_credentials', ['ssh_key_fingerprint'], unique=False)
    ...
    op.create_table('host_group_memberships',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('host_id', postgresql.UUID(as_uuid=True), nullable=False),
