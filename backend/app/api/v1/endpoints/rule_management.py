...
@router.get("/{rule_id}", response_model=APIResponse, summary="Get rule details with inheritance")
    ...
    try:
        if include_dependencies:
            ...
            return APIResponse(
                success=True,
                data=rule_detail,
                message=f"Retrieved rule {rule_id}"
            )
...
@router.post("/platform-capabilities", response_model=APIResponse, summary="Detect platform capabilities")
async def detect_platform_capabilities(
    ...
    try:
        ...
        baseline = await platform_svc.get_platform_baseline(
            capability_query.platform,
            capability_query.platform_version
        )
...
@router.post("/applicable", response_model=APIResponse, summary="Get applicable rules for capabilities")
...
async def get_rule_statistics(
    ...
    try:
        ...
        return APIResponse(
            success=True,
            data=statistics,
            message="Retrieved rule statistics"
        )
    ...
    try:
        ...
        if export_options.format == "json":
            ...
            if rules:
                ...
                fieldnames = ['rule_id', 'severity', 'category', 'platforms', 'frameworks']
    ...
    try:
        if service.cache_service:
            ...
            return APIResponse(
                success=True,
                data=cache_info,
                message="Retrieved cache information"
            )
        else:
            return APIResponse(
                success=False,
