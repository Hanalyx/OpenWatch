...
async def generate_recommendations(args):
    ...
    # Export if requested
    if args.export:
        output_file = args.output_file or f"remediation_recommendations_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
...
Examples:
  ...
  python -m backend.app.cli.remediation_recommendations orsa-mapping \\
    ...
    gaps_parser.add_argument('--scan-results', required=True,
                            help='JSON file containing scan results')
    gaps_parser.add_argument('--rules-directory', required=True,
                            help='Directory containing unified rules JSON files')
    gaps_parser.add_argument('--frameworks',
                            help='Comma-separated list of target frameworks')
    gaps_parser.add_argument('--verbose', action='store_true',
                            help='Show detailed gap information')
    gaps_parser.add_argument('--max-display', type=int, default=10,
                            help='Maximum gaps to display per priority')
    gaps_parser.add_argument('--export', action='store_true',
                            help='Export compliance gaps to JSON')
    gaps_parser.add_argument('--output-file',
    ...
    gen_parser.add_argument('--scan-results', required=True,
                           help='JSON file containing scan results')
    gen_parser.add_argument('--rules-directory', required=True,
                           help='Directory containing unified rules JSON files')
    gen_parser.add_argument('--frameworks',
                           help='Comma-separated list of target frameworks')
    gen_parser.add_argument('--min-priority', 
    ...
    gen_parser.add_argument('--max-display', type=int, default=10,
                           help='Maximum recommendations to display per priority')
    gen_parser.add_argument('--export', action='store_true',
                           help='Export recommendations to JSON')
    gen_parser.add_argument('--output-file',
                           help='Output file for exported data')
    
    # ORSA mapping command
    orsa_parser = subparsers.add_parser('orsa-mapping', help='Generate ORSA-compatible rule mappings')
    orsa_parser.add_argument('--scan-results', required=True,
                            help='JSON file containing scan results')
    orsa_parser.add_argument('--rules-directory', required=True,
                            help='Directory containing unified rules JSON files')
    orsa_parser.add_argument('--frameworks',
                            help='Comma-separated list of target frameworks')
    orsa_parser.add_argument('--verbose', action='store_true',
                            help='Show detailed ORSA rule information')
    orsa_parser.add_argument('--max-display', type=int, default=10,
                            help='Maximum rules to display per platform')
    orsa_parser.add_argument('--output-file',
                            help='Output file for ORSA mappings (JSON)')
