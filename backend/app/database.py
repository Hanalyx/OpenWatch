...
class Host(Base):  # type: ignore[valid-type, misc]
    ...
    # Phase 4: Platform identifier for OVAL selection
    # Normalized platform ID (e.g., "rhel9", "ubuntu2204") derived from os_family + os_version.
    # Used by mongodb_scan_api.py to select platform-specific OVAL definitions during scans.
    # Computed via _normalize_platform_identifier() in os_discovery_tasks.py.
    platform_identifier = Column(String(50), nullable=True, index=True)
...
class ScanBaseline(Base):  # type: ignore[valid-type, misc]
    ...
    baseline_type = Column(
        String(20), nullable=False, comment="Baseline type: initial, manual, or rolling_avg"
    )
