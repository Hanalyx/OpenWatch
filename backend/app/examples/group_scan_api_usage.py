...
class GroupScanAPIClient:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=self.headers, json=scan_config or {}) as response:
                if response.status == 200:
                    ...
                    raise Exception(
                        f"Failed to initiate group scan: {response.status} - {error_text}"
                    )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(
                        f"Failed to get scan progress: {response.status} - {error_text}"
                    )
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=self.headers) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(
                        f"Failed to cancel group scan: {response.status} - {error_text}"
                    )
    ...
    async def list_scan_sessions(
        self, status: str = None, group_id: int = None, limit: int = 20
    ) -> Dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=self.headers, params=params) as response:
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    raise Exception(
                        f"Failed to list scan sessions: {response.status} - {error_text}"
                    )
...
async def example_group_scan_workflow():
    ...
    client = GroupScanAPIClient(base_url="http://localhost:8000", auth_token="your_jwt_token_here")
    ...
    try:
        ...
        while True:
            ...
            print(
                f"   Scanning: {progress['hosts_scanning']}, "
        ...
        for host in host_details:
            ...
            if host["scan_results"]:
                ...
                print(
                    f"   Results: {results['passed_rules']}/{results['total_rules']} passed, "
    ...
    try:
        ...
        if active_scans:
            ...
            for scan in active_scans:
                print(
                    f"   {scan['session_id']}: {scan['group_name']} "
        ...
        for session in sessions["sessions"]:
            print(
                f"   {session['session_id'][:8]}... - {session['group_name']} "
