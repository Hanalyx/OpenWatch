...
class AuthorizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            ...
            if not current_user:
                logger.warning(
                    f"Authorization failed: No authenticated user for {request.method} {request.url.path}"
                )
                return self._create_error_response(
                    status.HTTP_401_UNAUTHORIZED, "Authentication required", request.url.path
                )
            ...
            if not resources:
                logger.warning(
                    f"Authorization failed: Could not extract resources from request {request.method} {request.url.path}"
                )
                return self._create_error_response(
            ...
            if authorization_result.overall_decision != AuthorizationDecision.ALLOW:
                logger.warning(
                    f"Authorization denied for user {current_user['id']} on {request.method} {request.url.path}: "
            ...
            logger.info(
                f"Authorization successful for user {current_user['id']} on {request.method} {request.url.path} "
    ...
    async def _extract_resources(
        self, request: Request, endpoint_config: Dict[str, Any], current_user: Dict[str, Any]
                ...
                if resource_type == ResourceType.HOST:
                    ...
                    if host_id:
                        ...
                        if body_host_id:
                            resources.append(
                                ResourceIdentifier(
                                    resource_type=ResourceType.HOST, resource_id=body_host_id
                                )
                            )
