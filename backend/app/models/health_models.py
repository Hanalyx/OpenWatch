...
class ServiceHealthDocument(Document):
    ...
    core_services: Dict[str, ServiceComponent] = Field(
        ...
        description="Core service components health"
    )
    data_services: Dict[str, DatabaseHealth] = Field(
        default_factory=dict,
        description="Database services health"
    )
    ...
    recent_operations: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Recent operation statistics"
    )
...
class ContentHealthDocument(Document):
    ...
    frameworks: Dict[str, FrameworkHealth] = Field(
        default_factory=dict,
        description="Framework health metrics"
    )
    
    # Benchmark health
    benchmarks: Dict[str, BenchmarkHealth] = Field(
        default_factory=dict,
        description="Benchmark health metrics"
    )
    
    # Rule statistics
    rule_statistics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Rule distribution and statistics"
    )
    
    # Content integrity
    content_integrity: ContentIntegrity = Field(
        default_factory=ContentIntegrity,
        description="Content integrity metrics"
    )
    
    # Performance metrics
    performance_metrics: Dict[str, Any] = Field(
        default_factory=dict,
        description="Content processing performance"
    )
    
    # Alerts and recommendations
    alerts_and_recommendations: List[ContentAlert] = Field(
        default_factory=list,
        description="Content alerts and recommendations"
    )
