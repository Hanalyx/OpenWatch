...
class PlatformImplementation(BaseModel):
    """
    Platform-specific implementation details for compliance rules.

    This model supports per-platform OVAL definition references (Option B schema)
    to enable accurate OS-specific compliance scanning. Each platform (rhel8, rhel9,
    ubuntu2204, etc.) can have its own OVAL file reference.

    The oval_filename field stores the relative path within /app/data/oval_definitions/
    (e.g., "rhel9/file_permissions_etc_passwd.xml").
    """
    ...
    # Per-platform OVAL definition reference (Option B schema)
    # Enables OS-aware OVAL selection during XCCDF generation
    oval_filename: Optional[str] = Field(
        default=None,
        description=(  # noqa: E501
            "Platform-specific OVAL XML definition filename relative to /app/data/oval_definitions/. "
            "Example: 'rhel9/package_firewalld_installed.xml'"
        ),
    )
...
class PlatformCapability(BaseModel):
    ...
    expected_exit_code: Optional[int] = Field(
        default=0, description="Expected exit code for successful capability detection"
    )
...
class XCCDFVariable(BaseModel):
    ...
    interactive: bool = Field(
        default=True,
        description=(
            "Whether this variable can be customized via UI/API (set to False for system variables)"  # noqa: E501
        ),
    )
    sensitive: bool = Field(
        default=False,
        description=(
            "Whether this variable contains sensitive data (passwords, keys, etc.). "
            "Encrypted in storage, masked in UI."
        ),
    )
...
class ComplianceRule(Document):
    ...
    # NOTE: Settings class is defined at the end of this Document class
    # to include all index definitions from the immutable versioning migration
    ...
    platform_requirements: Optional[Dict[str, Any]] = Field(
        default=None, description="Required and optional platform capabilities"
    )

    platform_capability_detection: Optional[Dict[str, PlatformCapability]] = Field(
        default=None, description="Configuration for detecting platform capabilities"
    )
    ...
    base_parameters: Optional[Dict[str, Any]] = Field(
        default=None, description="Base parameters that can be inherited and overridden"
    )
    inheritable_properties: Optional[Dict[str, Any]] = Field(
        default=None, description="Properties that derived rules inherit"
    )

    # Platform-Specific Parameter Overrides
    parameter_overrides: Optional[Dict[str, Dict[str, Any]]] = Field(
        default=None, description="Platform and framework-specific parameter overrides"
    )
    ...
    oval_filename: Optional[str] = Field(
        default=None,
        description=(
            "OVAL XML definition filename (stored in /app/data/oval_definitions/{platform}/). "
            "Example: 'account_disable_post_pw_expiration.xml'"
        ),
    )
    ...
    change_summary: Optional[Dict[str, Any]] = Field(
        default=None, description="Summary of what changed from previous version"
    )
    ...
    xccdf_variables: Optional[Dict[str, XCCDFVariable]] = Field(
        ...
        See: /docs/REMEDIATION_WITH_XCCDF_VARIABLES.md  # noqa: E501
        """,
    )
    ...
    scanner_type: str = Field(
        default="oscap",
        pattern=(
            "^(oscap|inspec|python|bash|aws_api|azure_api|gcp_api|kubernetes|docker|sql|"
            "mongodb|elasticsearch|opa_rego|custom)$"
        ),
        description="""
    ...
    remediation: Optional[Dict[str, Any]] = Field(
        default=None,
        description="""
        Remediation content for ORSA (Open Remediation Standard Adapter) plugins.  # noqa: E501

        Supports multiple remediation formats extracted from XCCDF or custom-defined:
        - ansible: Ansible tasks with variable bindings
        - bash: Bash scripts with variable substitution  # noqa: E501
    ...
    class Settings:
        ...
        indexes = [
            ...
            [
                ("is_latest", 1),
                ("platform_implementations.ubuntu.versions", 1),
                ("severity", -1),
            ],
            [
                ("is_latest", 1),
                ("platform_implementations.windows.versions", 1),
                ("severity", -1),
            ],
            ...
            IndexModel(
                ...
                weights={"metadata.name": 10, "metadata.description": 5, "tags": 2},
            ),
        ]
...
class RuleIntelligence(Document):
    ...
    rule_id: str = Field(description="Reference to compliance rule")
...
class RemediationScript(Document):
    ...
    rule_id: str = Field(description="Reference to compliance rule")
    platform: str = Field(description="Target platform (rhel, ubuntu, windows, etc.)")
