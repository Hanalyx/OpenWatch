...
class InstalledPlugin(Document):
    ...
    class Settings:
        ...
        indexes = [
            IndexModel([("manifest.name", ASCENDING), ("manifest.version", ASCENDING)], unique=True),
    ...
    def is_active(self) -> bool:
        ...
        return (
            ...
            self.trust_level != PluginTrustLevel.UNTRUSTED
        )
    ...
    def get_risk_score(self) -> int:
        ...
        critical_failures = sum(1 for check in self.security_checks 
                               if not check.passed and check.severity == "critical")
        high_failures = sum(1 for check in self.security_checks 
