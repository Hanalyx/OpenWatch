"""
Aegis Plugin Exceptions

Defines exception hierarchy for Aegis plugin operations.
...
class AegisError(Exception):
    """
    Base exception for all Aegis plugin errors.

    All Aegis-specific exceptions inherit from this class,
    allowing callers to catch all Aegis errors with a single handler.
    """
...
class AegisRuleLoadError(AegisError):
    """
    Error loading Aegis rules from YAML files.
...
class AegisLicenseError(AegisError):
    """
...
class AegisRollbackError(AegisError):
    """
...
class AegisUpdateError(AegisError):
    """
    Error during Aegis update.

    Raised when checking for or applying Aegis updates fails.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            detail: Description of the failure.
            current_version: Current Aegis version if known.
        ...
        message = f"Aegis update {operation} failed: {detail}"
