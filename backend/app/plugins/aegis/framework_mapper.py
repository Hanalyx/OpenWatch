"""
Aegis Framework Mapper Service

Maps Aegis rules to compliance framework controls (CIS, STIG, NIST).
Provides framework coverage statistics and control-to-rule lookups.

This service queries PostgreSQL tables (not MongoDB):
- aegis_rules: Rule metadata
- framework_mappings: Control mappings

Usage:
    from app.plugins.aegis.framework_mapper import FrameworkMapper
...
class FrameworkMapper:
    """
    Maps Aegis rules to compliance framework controls.
    ...
    async def get_rules_for_framework(
        ...
        """
        Get all Aegis rules mapped to a framework.
        ...
        if severity:
            severity_clause = "AND ar.severity = :severity"
            params["severity"] = severity

        if category:
            category_clause = "AND ar.category = :category"
        ...
        query = text(
            ...
            FROM aegis_rules ar
            INNER JOIN framework_mappings fm ON ar.rule_id = fm.aegis_rule_id
            WHERE fm.framework = :framework
            {version_clause}
            {severity_clause}
            {category_clause}
            ORDER BY ar.category, ar.rule_id
        """
        )
    ...
    async def get_framework_coverage(
        ...
        rules_query = text(
            f"""
            SELECT COUNT(DISTINCT aegis_rule_id)
            FROM framework_mappings
            WHERE framework = :framework
            {version_clause}
        """
        ...
        severity_query = text(
            f"""
            SELECT ar.severity, COUNT(DISTINCT ar.rule_id) as count
            FROM aegis_rules ar
            INNER JOIN framework_mappings fm ON ar.rule_id = fm.aegis_rule_id
            WHERE fm.framework = :framework
            {version_clause}
        ...
        category_query = text(
            f"""
            SELECT ar.category, COUNT(DISTINCT ar.rule_id) as count
            FROM aegis_rules ar
            INNER JOIN framework_mappings fm ON ar.rule_id = fm.aegis_rule_id
            WHERE fm.framework = :framework
            {version_clause}
    ...
    async def list_frameworks(self) -> List[Dict[str, Any]]:
        ...
        query = text(
            ...
            SELECT
                ...
                COUNT(DISTINCT aegis_rule_id) as rules
            FROM framework_mappings
