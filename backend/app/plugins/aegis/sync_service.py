"""
Aegis Rule Sync Service

Synchronizes Aegis YAML rules to PostgreSQL for fast querying.
This replaces MongoDB rule storage as part of the MongoDB deprecation.

The sync process:
1. Loads all YAML rules from backend/aegis/rules/
2. Computes file hashes for change detection
3. Upserts rules to aegis_rules table
4. Extracts framework mappings to framework_mappings table

Usage:
    from app.plugins.aegis.sync_service import AegisRuleSyncService
...
class AegisRuleSyncService:
    """
    Synchronizes Aegis YAML rules to PostgreSQL.

    This service:
    - Loads rules from backend/aegis/rules/
    - Upserts to aegis_rules table (PostgreSQL)
    ...
    def __init__(self, db: Session):
        ...
        """
        self.db = db
        self.rules_path = AEGIS_RULES_PATH

    def sync_all_rules(self, force: bool = False) -> Dict[str, Any]:
        """
        Sync all Aegis rules to PostgreSQL.
        ...
        logger.info(
            "Aegis rule sync complete: %d synced, %d skipped, %d mappings",
            stats["rules_synced"],
            stats["rules_skipped"],
    ...
    def _load_all_rules(self) -> List[Dict[str, Any]]:
        ...
        if not self.rules_path.exists():
            logger.warning("Aegis rules path not found: %s", self.rules_path)
            return rules
    ...
    def _upsert_rule(self, rule_data: Dict[str, Any]) -> None:
        """Upsert a rule to the aegis_rules table."""
        import json
        ...
        query = text(
            ...
            INSERT INTO aegis_rules (
                ...
                aegis_version, file_path, file_hash, created_at, updated_at
            )
            VALUES (
                :rule_id, :title, :description, :rationale, :severity, :category,
                :tags, :platforms, :references, :implementations,
                :aegis_version, :file_path, :file_hash, NOW(), NOW()
            )
            ON CONFLICT (rule_id) DO UPDATE SET
                ...
                aegis_version = EXCLUDED.aegis_version,
    ...
    def _insert_cis_mapping(self, rule_id: str, version: str, mapping: Dict[str, Any]) -> None:
        ...
        query = text(
            ...
            INSERT INTO framework_mappings (
                ...
                cis_level, cis_type, aegis_rule_id, created_at
            )
            VALUES (
    ...
    def _insert_stig_mapping(self, rule_id: str, version: str, mapping: Dict[str, Any]) -> None:
        ...
        query = text(
            ...
            INSERT INTO framework_mappings (
                ...
                vuln_id, cci, severity, aegis_rule_id, created_at
            )
            VALUES (
    ...
    def _insert_nist_mapping(self, rule_id: str, control_id: str) -> None:
        ...
        query = text(
            """
            INSERT INTO framework_mappings (
                framework, framework_version, control_id, aegis_rule_id, created_at
            )
    ...
    def get_sync_stats(self) -> Dict[str, Any]:
        ...
        rules_query = text("SELECT COUNT(*) FROM aegis_rules")
...
__all__ = ["AegisRuleSyncService", "AEGIS_VERSION"]
