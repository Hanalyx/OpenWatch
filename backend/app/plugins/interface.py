...
class RemediationPlugin(PluginInterface):
    ...
    @abstractmethod
    async def execute_remediation(self, rule_id: str, host_config: Dict, 
                                scan_result: ScanResult) -> Dict:
        """Execute remediation for a failed rule"""
        pass
    
    @abstractmethod
    async def get_remediation_plan(self, failed_rules: List[str], 
...
class IntegrationPlugin(PluginInterface):
    ...
    @abstractmethod
    async def export_results(self, scan_results: List[ScanResult], 
...
class AuthenticationPlugin(PluginInterface):
    ...
    @abstractmethod
    async def authorize_action(self, user_info: Dict, action: str, 
                             resource: str) -> bool:
...
def create_scan_context(scan_id: str, profile_id: str, content_path: str,
                       target_host: str, scan_type: str, **kwargs) -> ScanContext:
...
def create_scan_result(scan_id: str, hostname: str, status: str, 
                      timestamp: str, **kwargs) -> ScanResult:
