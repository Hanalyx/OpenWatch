"""
Kensa Plugin Updater

Handles Kensa independent updates with:
...
from app.plugins.kensa.config import KensaConfig, get_kensa_config
...
class KensaUpdater:
    ...
    def __init__(self, db: Session, config: Optional[KensaConfig] = None):
        """
        Initialize the updater.

        Args:
            db: Database session for tracking updates
            config: Optional Kensa configuration (uses global if not provided)
        """
        self.db = db
    ...
    async def perform_update(
        ...
        try:
            ...
            logger.info(f"Successfully updated Kensa from {current_version} to {version}")
    ...
    async def _download_package(self, version: str) -> Tuple[Path, str]:
        ...
        package_url = f"{self.registry_url}/packages/{version}/kensa-{version}.tar.gz"
        checksum_url = f"{self.registry_url}/packages/{version}/kensa-{version}.sha256"
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(package_url) as response:
                ...
                package_path = temp_dir / f"kensa-{version}.tar.gz"
    ...
    async def _backup_current(self, current_version: str) -> Path:
        ...
        backup_path = backup_base / f"kensa.backup.{current_version}.{timestamp}"

        # For pip-installed Kensa, backup the rules directory
        try:
            from runner.paths import get_rules_path

            rules_path = Path(get_rules_path())
            if rules_path.exists():
                shutil.copytree(rules_path, backup_path / "rules")
                logger.info(f"Created backup at {backup_path}")
        except ImportError:
            logger.warning("Cannot backup: runner.paths not available")
    ...
    async def _install_package(self, package_path: Path, manifest: Dict[str, Any]) -> None:
        ...
        logger.info(f"Installed Kensa version {manifest.get('version')}")
    ...
    async def _sync_rules_to_database(self) -> None:
        ...
        try:
            ...
            sync_service = KensaRuleSyncService(self.db)
            sync_service.sync_all_rules()
    ...
    def _create_update_record(
        ...
        query = """
            INSERT INTO plugin_updates (
                ...
                :id, 'kensa', :from_version, :to_version, 'pending', :user_id
            )
        """
        self.db.execute(
            text(query),
    ...
    async def _store_update_notification(
        ...
        query = """
            INSERT INTO plugin_update_notifications (
                ...
                'kensa', :current, :available, :min_ow,
