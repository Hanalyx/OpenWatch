...
ROLE_PERMISSIONS: Dict[UserRole, List[Permission]] = {
    UserRole.SUPER_ADMIN: [
        ...
        Permission.SCAN_APPROVE,
        Permission.SCAN_ROLLBACK,
    ...
    UserRole.SECURITY_ANALYST: [
        ...
        Permission.HOST_READ,
        Permission.HOST_UPDATE,  # Can manage assigned hosts
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not RBACManager.has_permission(user_role, permission):
                logger.warning(
                    f"User {current_user.get('username')} with role {user_role} attempted to access {permission}"
                )
                raise HTTPException(
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if not RBACManager.has_any_permission(user_role, permissions):
                logger.warning(
                    f"User {current_user.get('username')} with role {user_role} attempted to access {[p.value for p in permissions]}"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if user_role not in required_roles:
                logger.warning(
                    f"User {current_user.get('username')} with role {user_role} attempted to access endpoint requiring {[r.value for r in required_roles]}"
                )
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
...
def check_permission_async(current_user: dict, required_permission: Permission, db: Any = None):
    ...
    logger.warning(
        f"User {current_user.get('username')} attempted to access {required_permission.value}"
    )
    raise HTTPException(
        status_code=status.HTTP_403_FORBIDDEN,
