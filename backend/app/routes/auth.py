...
async def login(request: LoginRequest, http_request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if user.locked_until and user.locked_until > datetime.utcnow():
            audit_logger.log_security_event(
                "AUTH_FAILURE", f"Login attempt with locked account: {request.username}", client_ip
            )
            await log_login_event(
                db=db,
                username=request.username,
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Account is temporarily locked"
            )
        ...
        if not pwd_context.verify(request.password, user.hashed_password):
            ...
            """
                ...
                {"attempts": failed_attempts, "locked_until": locked_until, "user_id": user.id},
            )
            db.commit()
