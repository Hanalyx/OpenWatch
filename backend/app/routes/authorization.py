...
async def get_host_permissions(
    host_id: str, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)
):
    """
...
async def check_bulk_permissions(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid action: {request.action}"
            )
