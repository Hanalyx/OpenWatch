...
async def discover_basic_system_bulk(
    ...
    if not valid_hosts:
        raise HTTPException(
            status_code=400, 
            detail="No valid hosts found for discovery"
        )
...
async def get_discovery_status(
    ...
    return {
        ...
        "discovery_complete": bool(
            host.os_family and host.os_family != 'Unknown' and
            host.os_version and host.os_version != 'Unknown' and
            host.architecture and host.architecture != 'Unknown'
        )
    }
