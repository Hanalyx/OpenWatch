...
async def discover_host_network_topology(
    ...
    try:
        ...
        logger.info(f"Network discovery completed for host {host.hostname}: "
...
@router.get("/hosts/{host_id}/network-security-assessment", response_model=NetworkSecurityAssessment)
...
async def get_network_discovery_capabilities(
    ...
    capabilities = {
        ...
        "dns_configuration": {
            ...
            "commands_used": ["cat /etc/resolv.conf", "systemd-resolve --status", "nslookup"],
            "information_gathered": [
                "DNS servers",
                "Search domains",
                "Resolution testing",
