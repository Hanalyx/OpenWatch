    ...
    try:
        ...
        for row in result:
            ...
            if row.latest_scan_id:
                ...
                host_data.last_scan = (
                    row.scan_completed_at.isoformat()
                    if row.scan_completed_at
                    else (row.scan_started_at.isoformat() if row.scan_started_at else None)
                )
...
async def create_host(
    host: HostCreate, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)
):
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to create host"
        )
...
async def get_host(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve host"
        )
...
async def update_host(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update host"
        )
...
async def delete_host(
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete host"
        )
...
async def delete_host_ssh_key(
    ...
    try:
        ...
        if not row.ssh_key_fingerprint:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="No SSH key found to delete"
            )
        ...
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete SSH key"
        )
