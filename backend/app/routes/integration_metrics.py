    ...
    try:
        ...
        for operation in set(list(last_hour.keys()) + list(last_day.keys())):
            ...
            if hour_data and day_data:
                performance_data[operation]["trends"] = {
                    "error_rate_trend": "up" if hour_data.error_rate > day_data.error_rate else "down" if hour_data.error_rate < day_data.error_rate else "stable",
                    "performance_trend": "better" if hour_data.average_duration < day_data.average_duration else "worse" if hour_data.average_duration > day_data.average_duration else "stable"
                }
        ...
        return {
            ...
            "summary": {
                ...
                "avg_error_rate_1h": sum(data["last_hour"]["error_rate"] for data in performance_data.values()) / len(performance_data) if performance_data else 0,
                "avg_error_rate_24h": sum(data["last_24h"]["error_rate"] for data in performance_data.values()) / len(performance_data) if performance_data else 0
            }
        }
    except Exception as e:
