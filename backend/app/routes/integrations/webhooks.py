...
This module provides webhook endpoint registration and delivery tracking.
...
Migration Status:
    - /api/webhooks/* -> /api/integrations/webhooks/*
"""
...
async def update_webhook_endpoint(
    ...
    Args:
        webhook_id: UUID of the webhook
        webhook_update: Updated configuration
        db: Database session
        current_user: Authenticated user context
    ...
    try:
        ...
        allowed_updates = {
            ...
            "webhook_secret": "secret_hash = :secret_hash",  # pragma: allowlist secret
        ...
        if webhook_update.secret is not None:
            updates.append(allowed_updates["webhook_secret"])
...
async def test_webhook_endpoint(
    ...
    Returns:
        Test webhook queued confirmation

    Raises:
        HTTPException: 404 if webhook not found or inactive
    """
    try:
        # Get webhook details
        webhook_result = db.execute(
