"""
Aegis Compliance Scanning Endpoints

This module provides API endpoints for the Aegis compliance engine integration.
Aegis provides SSH-based compliance scanning with 338 canonical YAML rules.

Scan Endpoints:
    POST /aegis                      - Execute Aegis compliance scan
    GET  /aegis/health               - Aegis engine health check
    GET  /aegis/compliance-state/{host_id} - Get compliance state for host

Framework Mapping Endpoints (PostgreSQL):
    GET  /aegis/frameworks           - List available frameworks (static)
    GET  /aegis/frameworks/db        - List frameworks from database
    GET  /aegis/rules/framework/{framework} - Get rules for a framework
    GET  /aegis/framework/{framework}/coverage - Get framework coverage stats
    GET  /aegis/rules/{rule_id}/framework-refs - Get framework refs for rule
    GET  /aegis/controls/search      - Search controls across frameworks
    GET  /aegis/controls/{framework}/{control_id} - Get rules for control

Sync Endpoints:
    GET  /aegis/sync-stats           - Get sync statistics
    POST /aegis/sync                 - Trigger manual rule sync
...
router = APIRouter(prefix="/aegis", tags=["Aegis Scanning"])
...
class AegisScanRequest(BaseModel):
    """Request model for Aegis compliance scan."""
...
class AegisScanResponse(BaseModel):
    """Response model for Aegis compliance scan."""
...
class AegisFramework(BaseModel):
...
class AegisFrameworksResponse(BaseModel):
    """List of available frameworks."""

    frameworks: List[AegisFramework]
...
@router.post("/", response_model=AegisScanResponse)
async def execute_aegis_scan(
    request: AegisScanRequest,
    db: Session = Depends(get_db),
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> AegisScanResponse:
    """
    Execute an Aegis compliance scan on a target host.

    This endpoint uses the Aegis compliance engine for SSH-based scanning
    ...
    Returns:
        AegisScanResponse with scan results and compliance score.
    ...
    logger.info(
        "Starting Aegis scan %s for host %s",
        scan_id,
        request.host_id,
        extra={
            "scan_id": scan_id,
    ...
    try:
        ...
        scan_name = request.name or f"Aegis Scan - {hostname} - {start_time.strftime('%Y-%m-%d %H:%M')}"

        # profile_id for Aegis uses the framework name or "aegis_all"
        profile_id = f"aegis_{request.framework}" if request.framework else "aegis_all"
        ...
        insert_builder = (
            ...
            .values(
                ...
                start_time,
                current_user.get("id"),
                f'{{"scanner": "aegis", "framework": "{request.framework or "all"}"}}',
        ...
        # Calculate severity counts (Aegis uses: critical, high, medium, low)
        ...
        logger.info(
            "Aegis scan %s completed: %d/%d passed (%.1f%%)",
            scan_id,
            passed,
            total,
            score,
        ...
        raise HTTPException(
            ...
            detail="Aegis compliance engine not available. Check installation.",
        )
...
@router.get("/frameworks", response_model=AegisFrameworksResponse)
async def list_aegis_frameworks(
    current_user: Dict[str, Any] = Depends(get_current_user),
) -> AegisFrameworksResponse:
    """
    List available Aegis compliance frameworks.
    ...
    return AegisFrameworksResponse(frameworks=frameworks)
...
async def aegis_health(
    ...
    """
    Check Aegis engine health and availability.
    ...
    try:
        ...
        rule_count = len(list(AEGIS_RULES_PATH.rglob("*.yml"))) if AEGIS_RULES_PATH.exists() else 0

        return {
            "status": "healthy",
            "aegis_version": aegis_version,
            "rules_path": str(AEGIS_RULES_PATH),
        ...
        return {
            "status": "unavailable",
            "error": f"Aegis package not installed: {e}",
        }
    except Exception as e:
        return {
            "status": "error",
...
async def list_frameworks_from_db(
    ...
    Returns framework information with rule counts from synced Aegis rules.
...
async def get_rules_for_framework(
    ...
    """
    Get all Aegis rules mapped to a compliance framework.
...
async def get_rule_framework_refs(
    ...
    """
    Get all framework references for a specific Aegis rule.

    Returns mappings to CIS, STIG, NIST 800-53, PCI-DSS, and SRG.

    Args:
        rule_id: Aegis rule ID.
...
async def get_control_rules(
    ...
    """
    Get all Aegis rules that implement a specific control.
...
async def get_sync_stats(
    ...
    """
    Get current Aegis rule sync statistics.
...
async def trigger_rule_sync(
    ...
    """
    Manually trigger Aegis rule sync to PostgreSQL.

    This reloads all YAML rules from aegis/rules/ and upserts them to the
    aegis_rules and framework_mappings tables.
    ...
    try:
        ...
        logger.info(
            "Manual Aegis rule sync triggered by user %s: %d rules, %d mappings",
            current_user.get("id"),
...
async def get_compliance_state(
    ...
    Returns all rule findings from the most recent completed Aegis scan,
    ...
    # Find the latest completed Aegis scan for this host
    ...
    if not scan_result:
        # No Aegis scans yet - return empty state
