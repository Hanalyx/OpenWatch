...
Endpoints:
    ...
    GET /{scan_id}/failed-rules  - Get failed rules for Kensa integration
...
Architecture Notes:
    ...
    - Failed rules endpoint optimized for Kensa integration
        ...
        if scan_data.get("status") == "completed" and not scan_data.get("result_file"):
            try:
                ...
                if findings:
                    ...
                    scan_data["rule_results"] = rule_results
                    logger.info(f"Added {len(rule_results)} Kensa scan findings from database")
            except Exception as e:
                logger.error(f"Error fetching Kensa scan findings: {e}")
                scan_data["rule_results"] = []
...
async def get_scan_failed_rules(
    ...
    """
    Get failed rules from a completed scan for Kensa integration.

    Extracts failed rules from XCCDF result files with metadata suitable
    for remediation workflows. Optimized for integration with Kensa.
                ...
                for rule_result in root.findall(".//xccdf:rule-result", namespaces):
                    ...
                    if result_elem is not None and result_elem.text == "fail":
                        ...
                        failed_rule = {
                            ...
                            "remediation_available": True,  # Assume remediation available for Kensa
                        }
