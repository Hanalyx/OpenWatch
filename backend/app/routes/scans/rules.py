"""
Rule-Specific Scanning Endpoints

This module provides endpoints for targeted scanning of specific SCAP rules,
remediation verification, and rule compliance information. Moved from
rule_scanning.py as part of Phase 2 API Standardization.

Endpoints:
    POST /rules/scan               - Scan specific rules
    POST /rules/rescan-failed      - Re-scan failed rules from previous scan
    POST /rules/verify-remediation - Verify rules after remediation
    GET  /rules/{rule_id}/history  - Get scan history for a rule
    GET  /rules/{rule_id}/compliance-info - Get compliance framework info for rule
    POST /rules/remediation-plan   - Create remediation plan for failed rules

Architecture Notes:
    - Uses MongoDB for rule compliance mapping
    - Integrates with AEGIS for remediation planning
    - Rule scan history stored in PostgreSQL rule_scan_history table
    - Uses framework mapper for compliance framework intelligence

Security Notes:
    - All endpoints require JWT authentication
    - Uses parameterized queries for SQL injection prevention
    - SCAP content paths validated before access
"""
...
router = APIRouter(tags=["Rule Scanning"])
...
class RemediationPlanRequest(BaseModel):
    """Request model for creating remediation plan."""
...
def _store_rule_scan_results(db: Session, scan_results: Dict[str, Any]) -> None:
    ...
    Args:
        ...
        scan_results: Dictionary containing scan results with rule_results list.
    """
    try:
...
def _store_remediation_plan(db: Session, plan: Any, created_by: int) -> None:
    ...
    Args:
        db: SQLAlchemy database session.
        plan: RemediationPlan object from AEGIS mapper.
...
def _update_remediation_plan_status(
    ...
    Args:
        db: SQLAlchemy database session.
        aegis_remediation_id: AEGIS job ID to update.
        verification_report: Report from verification scan.
    """
    try:
...
def _analyze_improvement(previous_scan_id: str, current_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        previous_scan_id: ID of previous scan to compare against.
...
async def scan_specific_rules(
    ...
    Performs a targeted scan of selected rules rather than a full profile scan.
    Useful for verification after remediation or focused compliance checks.

    Args:
        request: Rule scan request with host_id, content_id, profile_id, rule_ids.
        db: SQLAlchemy database session.
        current_user: Authenticated user from JWT token.

    Returns:
        Dictionary with scan results, remediation recommendations, and summary.

    Raises:
        HTTPException 404: SCAP content not found.
    ...
    try:
        ...
        return {
            ...
            "remediation_recommendations": remediation_priorities[:10],
            "summary": {
...
@router.post("/rules/rescan-failed")
async def rescan_failed_rules(
    ...
    Useful for verification after remediation to check if fixes were effective.

    Args:
        previous_scan_id: UUID of the previous scan to get failed rules from.
        content_id: SCAP content ID.
        connection_params: Optional SSH connection parameters.
        db: SQLAlchemy database session.
        current_user: Authenticated user from JWT token.

    Returns:
        Dictionary with scan results and improvement analysis.

    Raises:
        HTTPException 404: SCAP content not found.
        HTTPException 500: Re-scan failure.
...
@router.post("/rules/verify-remediation")
async def verify_remediation(
    ...
    Scans the remediated rules to verify that fixes were applied successfully.

    Args:
        request: Verification request with host_id, content_id, remediation details.
        db: SQLAlchemy database session.
        current_user: Authenticated user from JWT token.

    Returns:
        Verification report with success/failure details.

    Raises:
        HTTPException 404: SCAP content not found.
        HTTPException 500: Verification failure.
...
async def get_rule_scan_history(
    ...
    Returns historical scan results for a rule, optionally filtered by host.

    Args:
        rule_id: XCCDF rule identifier.
        host_id: Optional host UUID to filter by.
        limit: Maximum number of results (default: 10).
        db: SQLAlchemy database session.
        current_user: Authenticated user from JWT token.

    Returns:
        Dictionary with rule_id, scan_history, and remediation_guidance.

    Raises:
        HTTPException 500: History retrieval failure.
...
@router.get("/rules/{rule_id}/compliance-info")
async def get_rule_compliance_info(
    ...
    Returns framework mappings, control information, and remediation details.
    ...
    Returns:
        Dictionary with rule info, framework mappings, and remediation availability.

    Raises:
        HTTPException 404: Rule not found in framework mappings.
        HTTPException 500: Information retrieval failure.
...
@router.post("/rules/remediation-plan")
async def create_remediation_plan(
    ...
    Generates an AEGIS-compatible remediation plan based on failed rules.

    Args:
        request: Plan request with scan_id, host_id, platform.
        db: SQLAlchemy database session.
        current_user: Authenticated user from JWT token.

    Returns:
        Dictionary with plan details and AEGIS job request.

    Raises:
        HTTPException 404: No failed rules found for scan.
        HTTPException 500: Plan creation failure.
