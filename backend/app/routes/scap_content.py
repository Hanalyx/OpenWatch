        ...
        try:
            ...
            """
                ),
                {
                    ...
                    "content_type": validation_result.get(
                        "content_type", validation_result.get("document_type", "unknown")
                    ),
...
async def get_scap_content(
    content_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)
):
...
async def download_scap_content(
    ...
    try:
        ...
        return FileResponse(
            path=result.file_path, filename=result.filename, media_type="application/octet-stream"
        )
...
async def get_repository_status(current_user: dict = Depends(get_current_user)):
...
async def sync_repositories(
    ...
    try:
        ...
        return {"message": "Repository sync completed", "results": results}
    except HTTPException:
        raise
    except Exception as e:
...
async def enable_repository(
    repository_id: str, enabled: bool = True, current_user: dict = Depends(get_current_user)
):
