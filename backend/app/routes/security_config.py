...
router = APIRouter(prefix="/api/v1/security/config", tags=["Security Configuration"])
...
async def validate_ssh_key(
    ...
    try:
        ...
        validator = get_credential_validator(
            policy_level=effective_config.policy_level, enforce_fips=effective_config.enforce_fips
        )
...
async def audit_credential(
    ...
    try:
        ...
        audit_result = validator.audit_credential_security(
            username=username, auth_method=auth_method, private_key=private_key, password=password
        )
