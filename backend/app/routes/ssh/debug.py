"""
SSH Debug API Endpoints

This module provides SSH debugging and diagnostics endpoints.
Part of Phase 4 API Standardization: System & Integrations.

Endpoint Structure:
    POST /debug/test-authentication  - Debug SSH authentication with diagnostics
    GET  /debug/paramiko-log         - Get paramiko debug log

Migration Status:
    - /api/ssh-debug/* -> /api/ssh/debug/*
"""
...
from .models import SSHDebugRequest, SSHDebugResponse

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/debug", tags=["SSH Debug"])
...
async def debug_ssh_authentication(
    ...
    Provides comprehensive SSH authentication testing including:
    - Host-specific credential testing
    - Global/system credential testing
    - SSH policy information
    - Recommendations for fixing issues

    Args:
        ssh_request: Debug request with host_id and options
        request: FastAPI request (for encryption service access)
        db: Database session
        current_user: Authenticated user context

    Returns:
        SSHDebugResponse with test results and recommendations

    Raises:
        HTTPException: 404 if host not found
        HTTPException: 500 if debug test fails
    """
    try:
        ...
        if ssh_request.test_host_credentials and host.encrypted_credentials:
            ...
            try:
                ...
                # Cast SQLAlchemy column types to Python native types
...
async def get_paramiko_debug_log(
    ...
    Provides access to the paramiko SSH protocol debug log for
    ...
    Raises:
        HTTPException: 500 if log read fails
    """
    try:
        import os
