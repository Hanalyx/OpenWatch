    ...
    try:
        ...
        if credentials.private_key and credentials.auth_method in ["ssh_key", "both"]:
            ...
            if validation_result.warnings:
                logger.warning(
                    f"SSH key warnings for system credentials '{credentials.name}': {sanitize_for_log('; '.join(validation_result.warnings))}"
                )
        ...
        if credentials.private_key:
            ...
            if metadata.get("error"):
                logger.warning(
                    f"Failed to extract SSH key metadata for '{credentials.name}': {sanitize_for_log(metadata.get('error'))}"
                )
...
async def get_default_credentials(
    ...
    add_deprecation_headers(
        response,
        "/api/v1/system/credentials/default",
...
async def update_system_credentials(
    ...
    add_deprecation_headers(
        response,
        f"/api/v1/system/credentials/{credential_id}",
        current_user.get("username", "unknown"),
    )
    try:
    ...
    try:
        ...
        if row.is_default:
            ...
            if count_result.fetchone().count <= 1:
                raise HTTPException(
                    status_code=400,
                    detail="Cannot delete the last active credential set",
                )
...
async def delete_ssh_key_from_credentials(
    ...
    add_deprecation_headers(
        response,
        f"/api/v1/system/credentials/{credential_id}/ssh-key",
        current_user.get("username", "unknown"),
    )
