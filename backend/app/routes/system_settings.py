...
async def list_system_credentials(
    ...
    try:
        ...
        credentials = []
        for row in result:
            credentials.append(SystemCredentialsResponse(
                id=row.id,
                name=row.name,
...
async def update_system_credentials(
    ...
    try:
        ...
        if credentials.private_key and credentials.auth_method in ["ssh_key", "both"]:
            ...
            if not validation_result.is_valid:
                logger.error(f"SSH key validation failed for system credentials update (ID: {credential_id}): {validation_result.error_message}")
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Invalid SSH key: {validation_result.error_message}"
                )
            ...
            # Log warnings if any
            if validation_result.warnings:
                logger.warning(f"SSH key warnings for system credentials update (ID: {credential_id}): {'; '.join(validation_result.warnings)}")
            
            # Log recommendations
            if validation_result.recommendations:
                logger.info(f"SSH key recommendations for system credentials update (ID: {credential_id}): {'; '.join(validation_result.recommendations)}")
        ...
        if credentials.private_key:
            ...
            if metadata.get('error'):
                logger.warning(f"Failed to extract SSH key metadata for update (ID: {credential_id}): {metadata.get('error')}")
        ...
        if credentials.private_key is not None:
            ...
            params["encrypted_private_key"] = encrypt_data(credentials.private_key.encode()) if credentials.private_key else None
        ...
        if credentials.private_key_passphrase is not None:
            ...
            params["private_key_passphrase"] = encrypt_data(credentials.private_key_passphrase.encode()) if credentials.private_key_passphrase else None
    ...
    try:
        ...
        if row.is_default:
            ...
            if count_result.fetchone().count <= 1:
                raise HTTPException(status_code=400, detail="Cannot delete the last active credential set")
...
async def start_scheduler(
    ...
    try:
        ...
        try:
            ...
            """), {
                "interval": str(request.interval_minutes),
                "updated_at": datetime.utcnow()
            })
...
async def list_alert_settings(
    ...
    try:
        ...
        settings = []
        for row in result:
            settings.append(AlertSettingsResponse(
                id=row.id,
                user_id=row.user_id,
