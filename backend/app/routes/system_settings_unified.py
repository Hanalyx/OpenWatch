...
async def create_system_credential(
    ...
    try:
        ...
        if not created_cred:
            logger.error(
                f"Failed to find credential {credential_id} in list of {len(created_cred_list)} credentials"
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            ...
            if config:
                ...
                if config.enabled and config.auto_start:
                    ...
                    if scheduler and not scheduler.running:
                        ...
                        logger.info(
                            f"Added new monitoring job with {_scheduler_interval} minute interval"
                        )
