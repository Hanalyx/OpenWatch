...
async def start_remediation(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Failed to start remediation"
        )
...
async def retry_remediation_job(
    ...
    try:
        ...
        if not scan:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Original remediation job not found"
            )
        ...
        if failed_rules_only and "results" in scan.metadata.get("remediation", {}):
            ...
            failed_rules = [
                r["rule_id"] for r in scan.metadata["remediation"]["results"]
                if r["status"] == "failed"
            ]
        else:
        ...
        if not failed_rules:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="No failed rules to retry"
            )
...
async def get_remediation_providers(
    ...
    try:
        ...
        providers.append(RemediationProvider(
            ...
            supported_os=[
                "rhel8", "rhel9", "ubuntu20.04", "ubuntu22.04",
                "centos8", "debian11"
            ],
...
async def get_remediation_summary(
    ...
    try:
        ...
        total_jobs = db.query(Scan).filter(
            Scan.remediation_requested == True
        ).count()
        
        active_jobs = db.query(Scan).filter(
            Scan.remediation_status.in_(["pending", "running"])
        ).count()
        
        completed_jobs = db.query(Scan).filter(
            Scan.remediation_status == "completed"
        ).count()
        
        failed_jobs = db.query(Scan).filter(
            Scan.remediation_status == "failed"
        ).count()
        
        pending_jobs = db.query(Scan).filter(
            Scan.remediation_status == "pending"
        ).count()
