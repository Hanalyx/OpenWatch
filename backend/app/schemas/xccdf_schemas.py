...
class XCCDFBenchmarkRequest(BaseModel):
    ...
    title: str = Field(
        ...,
        description="Human-readable benchmark title"
    )
    ...
    version: str = Field(
        ...,
        description="Benchmark version string (e.g., '1.0.0')"
    )
    ...
    framework_version: Optional[str] = Field(
        None,
        description="Specific framework version (e.g., '800-53r5', 'v2.0.0')"
    )
    rule_filter: Optional[Dict] = Field(
        None,
        description="Additional MongoDB query filter for rules"
    )
...
class XCCDFBenchmarkResponse(BaseModel):
    ...
    benchmark_xml: str = Field(
        ...,
        description="Generated XCCDF Benchmark XML content"
    )
    rules_count: int = Field(
        ...,
        description="Number of rules included in benchmark"
    )
    variables_count: int = Field(
        ...,
        description="Number of XCCDF variables defined"
    )
    profiles_count: int = Field(
        ...,
        description="Number of profiles created"
    )
...
class XCCDFTailoringRequest(BaseModel):
    ...
    tailoring_id: str = Field(
        ...,
        description="Unique tailoring identifier"
    )
    ...
    profile_id: str = Field(
        ...,
        description="Base profile to customize"
    )
    ...
    title: Optional[str] = Field(
        None,
        description="Custom title for tailored profile"
    )
    description: Optional[str] = Field(
        None,
        description="Description of customizations"
    )
...
class XCCDFTailoringResponse(BaseModel):
    ...
    tailoring_xml: str = Field(
        ...,
        description="Generated XCCDF Tailoring XML content"
    )
    variables_overridden: int = Field(
        ...,
        description="Number of variables customized"
    )
    generated_at: str = Field(
        ...,
        description="ISO 8601 timestamp of generation"
...
class XCCDFValidationRequest(BaseModel):
    ...
    xccdf_content: str = Field(
        ...,
        description="XCCDF XML content to validate"
    )
...
class XCCDFValidationResponse(BaseModel):
    ...
    valid: bool = Field(
        ...,
        description="Whether the XCCDF is valid"
    )
