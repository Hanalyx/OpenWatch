...
from ..models.authorization_models import (
    ResourceType, ActionType, PermissionEffect, PermissionScope,
    AuthorizationDecision, ResourceIdentifier, AuthorizationContext,
    AuthorizationResult, BulkAuthorizationRequest, BulkAuthorizationResult,
    HostPermission, HostGroupPermission, AuthorizationAuditEvent,
    PolicyConflictResolution, AuthorizationConfiguration, PermissionCache
)
from ..rbac import UserRole, RBACManager, Permission
...
class AuthorizationService:
    ...
    async def check_bulk_permissions(
        ...
        logger.info(f"Bulk authorization check for user {request.user_id}: {len(request.resources)} resources, action {request.action}")
        ...
        try:
                ...
                for resource in request.resources:
                    ...
                    if request.fail_fast and result.decision == AuthorizationDecision.DENY:
                        ...
                        for remaining_resource in remaining_resources:
                            individual_results.append(AuthorizationResult(
                                decision=AuthorizationDecision.DENY,
            ...
            if self.config.enable_audit_logging:
                await self._audit_bulk_authorization(
                    request, overall_decision, len(allowed_resources), len(denied_resources), total_time
                )
            ...
            logger.info(f"Bulk authorization completed: {overall_decision.value} "
    ...
    def _get_applicable_policies(
        ...
        try:
            ...
            logger.debug(f"Found {len(policies)} applicable policies for user {user_id} on resource {resource.resource_id}")
            return policies
    ...
    def _evaluate_policies(self, policies: List[Dict]) -> Tuple[AuthorizationDecision, str]:
        ...
        if self.config.conflict_resolution == PolicyConflictResolution.DENY_OVERRIDES:
            if deny_policies:
                return AuthorizationDecision.DENY, f"Access explicitly denied by {len(deny_policies)} deny policies"
            elif allow_policies:
                return AuthorizationDecision.ALLOW, f"Access granted by {len(allow_policies)} allow policies"
        
        elif self.config.conflict_resolution == PolicyConflictResolution.ALLOW_OVERRIDES:
            if allow_policies:
                return AuthorizationDecision.ALLOW, f"Access granted by {len(allow_policies)} allow policies"
            elif deny_policies:
                return AuthorizationDecision.DENY, f"Access denied by {len(deny_policies)} deny policies"
    ...
    def _evaluate_role_permissions(
        ...
        try:
            ...
            if not required_permission:
                return AuthorizationDecision.DENY, f"No role permission mapping for action {action}"
            ...
            for role_name in context.user_roles:
                try:
                    ...
                    if RBACManager.has_permission(user_role, required_permission):
                        return AuthorizationDecision.ALLOW, f"Role {role_name} has permission {required_permission.value}"
                except ValueError:
                    logger.warning(f"Unknown role: {role_name}")
                    continue
            
            return AuthorizationDecision.DENY, f"No user role has permission {required_permission.value}"
    ...
    def _combine_decisions(
        ...
        if policy_allow and role_allow:
            return AuthorizationDecision.ALLOW, f"Both policy and role checks passed: {policy_decision[1]} AND {role_decision[1]}"
        
        # If either denies, deny access
        if not policy_allow and not role_allow:
            return AuthorizationDecision.DENY, f"Both policy and role checks failed: {policy_decision[1]} AND {role_decision[1]}"
        elif not policy_allow:
            return AuthorizationDecision.DENY, f"Policy check failed: {policy_decision[1]}"
        else:
        ...
        try:
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    ...
                    valid_results.append(AuthorizationResult(
                        decision=AuthorizationDecision.DENY,
