...
class AuthorizationService:
    ...
    def __init__(self, db: Session, config: Optional[AuthorizationConfiguration] = None):
        ...
        self.permission_cache = PermissionCache(
            ttl_seconds=self.config.cache_ttl_seconds, max_size=self.config.max_cache_size
        )
    ...
    async def check_permission(
        ...
        try:
            ...
            if result.decision == AuthorizationDecision.DENY:
                logger.warning(
                    f"ACCESS DENIED: User {user_id} denied {action} on {resource.resource_type}:{resource.resource_id} - {result.reason}"
                )
            else:
                logger.debug(
    ...
    def _build_user_context(self, user_id: str) -> AuthorizationContext:
        ...
        try:
            ...
            return AuthorizationContext(
                user_id=user_id, user_roles=[row.role] if row.role else [], user_groups=user_groups
            )
