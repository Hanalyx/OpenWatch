...
from .scan_intelligence import ScanIntelligenceService, HostInfo
...
class BulkScanOrchestrator:
    ...
    async def create_bulk_scan_session(
        ...
        try:
            ...
            if not authorized_hosts:
                logger.warning(f"User {user_id} has no authorized hosts for bulk scan. {len(authorization_failures)} failures.")
                raise PermissionError(f"No hosts authorized for scanning. Access denied to all {len(host_ids)} requested hosts.")
            
            # Log authorization summary
            logger.info(f"Authorization complete: {len(authorized_hosts)} authorized, {len(authorization_failures)} denied")
            ...
            session = ScanSession(
                ...
                scan_ids=[],
                estimated_completion=datetime.utcnow() + timedelta(minutes=feasibility.get("estimated_time_minutes", 60)),
            ...
            for batch in scan_plan:
                ...
                batch_scan_ids = await self._create_batch_scans_with_authorization(
                    batch, session_id, name_prefix, user_id, stagger_delay, authorized_hosts
                )
            ...
            if authorization_failures:
                logger.warning(f"Bulk scan session {session_id} created with {len(scan_ids)} scans. "
                              f"Authorization denied for {len(authorization_failures)} hosts.")
            else:
                logger.info(f"Bulk scan session {session_id} created successfully with {len(scan_ids)} scans. "
    ...
    async def _plan_bulk_scan(self, host_ids: List[str], template_id: str, session_id: str, priority: str) -> List[ScanBatch]:
        ...
        try:
            ...
            for result in auth_result.individual_results:
                ...
                if result.decision == AuthorizationDecision.ALLOW:
                    authorized_hosts.append(AuthorizedHost(
                        host_id=host_id,
                    ...
                    authorization_failures.append(AuthorizationFailure(
                        host_id=host_id,
                        hostname=host_detail.get('hostname', 'unknown'),
            ...
            logger.info(f"Bulk authorization results for user {user_id}: "
