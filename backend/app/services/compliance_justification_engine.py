...
from backend.app.services.framework_mapping_engine import ControlMapping, MappingType, MappingConfidence
...
Execution Time: {rule_execution.execution_time:.3f} seconds
    ...
    async def _analyze_exceeding_compliance(
        ...
        for mapping in unified_rule.framework_mappings:
            if mapping.framework_id != framework_id and mapping.implementation_status in ["compliant", "exceeds"]:
...
security standards and meeting audit requirements.
    ...
    async def _generate_impact_analysis(
        ...
        if rule_execution.compliance_status in [ComplianceStatus.COMPLIANT, ComplianceStatus.EXCEEDS]:
            return f"""
        ...
        if export_format == "json":
            audit_package = {
                ...
                "compliance_summary": {
                    ...
                    "non_compliant": len([j for j in justifications if j.compliance_status == ComplianceStatus.NON_COMPLIANT])
                },
                "justifications": [
                    {
