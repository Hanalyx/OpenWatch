...
Handles immutable versioning, content hashing, and change detection
for FISMA/FedRAMP/HIPAA compliance requirements.

Features:
    - Content hashing (SHA-256) for tamper detection
    - Change detection between rule versions
    - Breaking change analysis for re-scan decisions
    - Audit trail with change summaries
...
BREAKING_CHANGE_FIELDS: Set[str] = {
    "check_content",
    "check_type",
    "severity",
    "frameworks",
...
class RuleVersioningService:
    ...
    def calculate_content_hash(rule_data: Dict[str, Any]) -> str:
        ...
        Example:
            >>> hash_val = RuleVersioningService.calculate_content_hash({
            ...     "rule_id": "test-rule",
            ...     "severity": "high",
            ... })
        ...
        content_dict = {k: v for k, v in rule_data.items() if k not in HASH_EXCLUDE_FIELDS}
    ...
    def has_breaking_changes(changes: Dict[str, Dict[str, Any]]) -> bool:
        ...
        Example:
            ...
            >>> RuleVersioningService.has_breaking_changes(changes)
            True
        """
        for changed_field in changes.keys():
            # Check top-level breaking fields
            if changed_field in BREAKING_CHANGE_FIELDS:
                return True

            # Check nested breaking fields (e.g., "dependencies.requires")
            for breaking_field in BREAKING_CHANGE_FIELDS:
    ...
    def create_change_summary(
        ...
        Returns:
            Change summary dictionary with:
    ...
    def verify_content_hash(rule_data: Dict[str, Any], expected_hash: str) -> bool:
        ...
        Example:
            >>> rule = {"rule_id": "test", "severity": "high"}
            >>> hash_val = RuleVersioningService.calculate_content_hash(rule)
            >>> RuleVersioningService.verify_content_hash(rule, hash_val)
            True
        """
    ...
    def prepare_new_version(
        ...
        Returns:
            Complete rule data ready for MongoDB insertion with:
            - Version number (1 for new, incremented for updates)
            - Content hash for integrity
            - Temporal fields (effective_from, imported_at)
            - Source tracking (bundle, hash, import_id)
    ...
    def get_field_change_description(field_name: str, old_value: Any, new_value: Any) -> str:
        ...
        Example:
            >>> RuleVersioningService.get_field_change_description(
            ...     "severity", "low", "high"
            ... )
