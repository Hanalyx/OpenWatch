#!/usr/bin/env python3
"""
Kensa Integration Mapper

Maps SCAP compliance rules to Kensa remediation actions and manages
automated remediation workflows. Kensa provides automated remediation
capabilities for compliance findings.

This module enables:
1. Rule-to-remediation mapping with category-based prioritization
2. Remediation plan generation with dependency resolution
3. Kensa job request generation for automated execution
4. Result mapping from Kensa back to SCAP rule status

Security Considerations:
- Commands stored in mappings are NOT executed by this module
- All command execution is delegated to Kensa or remediation services
- No shell=True subprocess calls in this module
- Input validation on all external data

Architecture:
- Single Responsibility: Maps between SCAP and Kensa formats only
...
Usage:
    ...
    # Generate Kensa job request
    job_request = mapper.generate_kensa_job_request(plan)
"""
...
_kensa_mapper_instance: Optional["KensaMapper"] = None


@dataclass
class KensaMapping:
    """
    Represents a mapping between a SCAP rule and Kensa remediation action.

    This dataclass contains all information needed to remediate a specific
    SCAP compliance finding through Kensa automation.

    Attributes:
        scap_rule_id: Full SCAP/XCCDF rule identifier
        kensa_rule_id: Corresponding Kensa rule identifier
    ...
    Note:
        Commands in this mapping are stored for Kensa execution only.
...
class RemediationPlan:
    ...
    Attributes:
        ...
        remediable_rules: Number of rules with Kensa mappings
    ...
    rule_groups: Dict[str, List[KensaMapping]]
...
class KensaMapper:
    """
    Service for mapping SCAP rules to Kensa remediation actions.

    This class provides the core integration between SCAP compliance
    scanning and Kensa automated remediation. It handles:

    1. Loading and caching rule mappings from configuration
    2. Creating remediation plans with proper execution ordering
    3. Generating Kensa job requests for automation
    4. Mapping Kensa results back to SCAP rule status
    ...
    Attributes:
        ...
        rule_mappings: Cached SCAP to Kensa mappings
        category_priorities: Priority order for remediation categories

    Example:
        mapper = KensaMapper(mappings_dir="/openwatch/data/mappings")
        mapping = mapper.get_kensa_mapping("xccdf_rule_ssh_config")
        if mapping:
            print(f"Kensa rule: {mapping.kensa_rule_id}")
    """

    def __init__(self, mappings_dir: str = "/openwatch/data/mappings") -> None:
        """
        Initialize the Kensa mapper with mapping directory.
        ...
        self.rule_mappings: Dict[str, KensaMapping] = self._load_mappings()
    ...
    def _load_mappings(self) -> Dict[str, KensaMapping]:
        """
        Load SCAP to Kensa mappings from all sources.
        ...
        Returns:
            Dictionary mapping SCAP rule IDs to KensaMapping objects.

        Note:
            Uses yaml.safe_load to prevent arbitrary code execution
            from malicious YAML files.
        """
        mappings: Dict[str, KensaMapping] = {}
        ...
        for mapping_file in self.mappings_dir.glob("*.yml"):
            try:
                ...
                for rule_id, mapping_data in custom_mappings.items():
                    ...
                    mappings[rule_id] = KensaMapping(
                        scap_rule_id=str(rule_id),
                        kensa_rule_id=str(mapping_data.get("kensa_rule_id", "")),
    ...
    def _load_builtin_mappings(self) -> Dict[str, KensaMapping]:
        """
        Load built-in SCAP to Kensa mappings for common rules.
        ...
        Returns:
            Dictionary of built-in KensaMapping objects.

        Note:
            Commands here are stored for reference and Kensa execution.
            This module does NOT execute any commands directly.
        """
        return {
            # SSH Configuration - Disable root login
            "xccdf_mil.disa.stig_rule_SV-230221r792832_rule": KensaMapping(
                scap_rule_id="xccdf_mil.disa.stig_rule_SV-230221r792832_rule",
                kensa_rule_id="RHEL-09-255045",
            ...
            "xccdf_mil.disa.stig_rule_SV-230365r792936_rule": KensaMapping(
                scap_rule_id="xccdf_mil.disa.stig_rule_SV-230365r792936_rule",
                kensa_rule_id="RHEL-09-611045",
                rule_category="authentication",
                remediation_type="configuration",
                implementation_commands=[
            ...
            "xccdf_mil.disa.stig_rule_SV-230423r793041_rule": KensaMapping(
                scap_rule_id="xccdf_mil.disa.stig_rule_SV-230423r793041_rule",
                kensa_rule_id="RHEL-09-653015",
                rule_category="audit",
            ...
            "xccdf_mil.disa.stig_rule_SV-230515r793185_rule": KensaMapping(
                scap_rule_id="xccdf_mil.disa.stig_rule_SV-230515r793185_rule",
                kensa_rule_id="RHEL-09-251010",
            ...
            "xccdf_mil.disa.stig_rule_SV-230279r792861_rule": KensaMapping(
                scap_rule_id="xccdf_mil.disa.stig_rule_SV-230279r792861_rule",
                kensa_rule_id="RHEL-09-232010",
    ...
    def get_kensa_mapping(self, scap_rule_id: str) -> Optional[KensaMapping]:
        """
        Get Kensa mapping for a specific SCAP rule.

        Args:
            scap_rule_id: Full SCAP/XCCDF rule identifier.

        Returns:
            KensaMapping if found, None otherwise.
    ...
    def create_remediation_plan(
        ...
        This method analyzes failed rules, matches them to Kensa mappings,
        ...
        for rule in failed_rules:
            ...
            mapping = self.get_kensa_mapping(rule_id)
    ...
    def generate_kensa_job_request(self, plan: RemediationPlan) -> Dict[str, Any]:
        """
        Generate Kensa job request from remediation plan.

        Creates a structured job request that can be submitted to
        Kensa for automated remediation execution.

        Args:
            plan: RemediationPlan containing rules to remediate.

        Returns:
            Dictionary containing Kensa job request specification.
        ...
        Example:
            job_request = mapper.generate_kensa_job_request(plan)
            # Submit to Kensa API
            response = kensa_client.submit_job(job_request)
        """
        if plan.remediable_rules == 0:
            raise ValueError("Plan has no remediable rules")

        # Extract Kensa rule IDs in execution order
        kensa_rules: List[str] = []
        for scap_rule_id in plan.execution_order:
            mapping = self.get_kensa_mapping(scap_rule_id)
            if mapping:
                kensa_rules.append(mapping.kensa_rule_id)

        # Build Kensa job request structure
        job_request: Dict[str, Any] = {
            "host_id": plan.host_id,
            "rule_ids": kensa_rules,
            "options": {
                "dry_run": False,
                "force": False,
    ...
    def map_kensa_results_to_scap(
        self,
        kensa_job_id: str,
        kensa_results: Dict[str, Any],
    ) -> Dict[str, str]:
        """
        Map Kensa remediation results back to SCAP rule status.

        After Kensa executes remediation, this maps the results
        back to SCAP rule IDs for updating scan status.

        Args:
            kensa_job_id: Kensa job identifier (for logging).
            kensa_results: Kensa job result containing execution status.

        Returns:
            Dictionary mapping SCAP rule IDs to status ("pass" or "fail").

        Example:
            results = mapper.map_kensa_results_to_scap(
                kensa_job_id="job-123",
                kensa_results={"executions": [{"rule_id": "RHEL-09-255045", "status": "completed"}]}
            )
            # Returns: {"xccdf_rule_ssh_config": "pass"}
        """
        scap_results: Dict[str, str] = {}

        # Get execution results from Kensa response
        executions = kensa_results.get("executions", [])

        for execution in executions:
            kensa_rule_id = execution.get("rule_id", "")
            status = execution.get("status", "unknown")

            # Find corresponding SCAP rule
            for scap_id, mapping in self.rule_mappings.items():
                if mapping.kensa_rule_id == kensa_rule_id:
                    # Map Kensa status to SCAP status
                    scap_results[scap_id] = "pass" if status == "completed" else "fail"
                    break

        logger.info(
            f"Mapped Kensa job {kensa_job_id} results: "
    ...
    def export_mappings(self, export_format: str = "yaml") -> str:
        """
        Export all SCAP to Kensa mappings.
        ...
        for scap_id, mapping in self.rule_mappings.items():
            export_data[scap_id] = {
                "kensa_rule_id": mapping.kensa_rule_id,
                "category": mapping.rule_category,
...
def get_kensa_mapper(mappings_dir: str = "/openwatch/data/mappings") -> KensaMapper:
    """
    Get or create the singleton KensaMapper instance.
    ...
    Returns:
        Singleton KensaMapper instance.

    Example:
        mapper = get_kensa_mapper()
        plan = mapper.create_remediation_plan(...)
    """
    global _kensa_mapper_instance

    if _kensa_mapper_instance is None:
        _kensa_mapper_instance = KensaMapper(mappings_dir=mappings_dir)
        logger.info("Initialized KensaMapper singleton")

    return _kensa_mapper_instance
