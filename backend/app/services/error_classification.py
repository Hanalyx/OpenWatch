...
from ..models.error_models import (
    ScanErrorInternal, ScanErrorResponse, ValidationResultInternal, ValidationResultResponse,
    ErrorCategory, ErrorSeverity
)
from .error_sanitization import get_error_sanitization_service, SanitizationLevel
            ...
            try:
                ...
                if result != 0:
                    errors.append(ScanErrorInternal(
                        ...
                        technical_details={"hostname": hostname, "port": port, "ip_address": ip_address, "connection_result": result},
                ...
                errors.append(ScanErrorInternal(
                    ...
                    technical_details={"hostname": hostname, "port": port, "timeout": 10},
...
class AuthenticationValidator:
    ...
    @staticmethod
    async def validate_credentials(hostname: str, port: int, username: str, 
                                 auth_method: str, credential: str) -> List[ScanErrorInternal]:
                ...
                try:
                    ...
                    if "too many authentication failures" in error_msg:
                        errors.append(ScanErrorInternal(
                            ...
                            technical_details={"username": username, "hostname": hostname},
                        ...
                        errors.append(ScanErrorInternal(
                            ...
                            technical_details={"username": username, "auth_method": auth_method},
...
class PrivilegeValidator:
    ...
    @staticmethod
    async def validate_privileges(ssh_client: paramiko.SSHClient) -> List[ScanErrorInternal]:
            ...
            if exit_status != 0:
                if "password is required" in stderr_output.lower():
                    errors.append(ScanErrorInternal(
                        ...
                        technical_details={"command": "sudo -n oscap --version", "stderr": stderr_output},
            ...
            if selinux_status == "enforcing":
                ...
                if "off" in bool_output:
                    errors.append(ScanErrorInternal(
                        ...
                        technical_details={"selinux_status": "enforcing", "openscap_can_network": "off"},
                ...
                try:
                    ...
                    if available_mb < cls.MIN_DISK_SPACE_MB:
                        errors.append(ScanErrorInternal(
                            ...
                            technical_details={"available_space_mb": available_mb, "required_space_mb": cls.MIN_DISK_SPACE_MB},
                ...
                try:
                    ...
                    if available_mem_mb < cls.MIN_MEMORY_MB:
                        errors.append(ScanErrorInternal(
                            ...
                            technical_details={"available_memory_mb": available_mem_mb, "recommended_memory_mb": cls.MIN_MEMORY_MB},
...
class ErrorClassificationService:
    ...
    async def classify_error(self, error: Exception, context: Dict[str, Any] = None) -> ScanErrorInternal:
        ...
        if any(keyword in error_str for keyword in ['permission denied', 'authentication failed', 'invalid credentials']):
            return ScanErrorInternal(
                error_code="AUTH_005",
                category=ErrorCategory.AUTHENTICATION,
                severity=ErrorSeverity.ERROR,
    ...
    async def validate_scan_prerequisites(self, hostname: str, port: int, username: str, 
                                        auth_method: str, credential: str,
                                        user_id: Optional[str] = None,
                                        source_ip: Optional[str] = None) -> ValidationResultInternal:
        ...
        try:
            auth_errors = await self.auth_validator.validate_credentials(hostname, port, username, auth_method, credential)
        ...
        try:
            ...
            stdin, stdout, stderr = ssh_client.exec_command('uname -a && cat /etc/os-release 2>/dev/null || echo "OS info not available"', timeout=10)
            ...
            validation_checks["privileges"] = len([e for e in privilege_errors if e.severity == ErrorSeverity.ERROR]) == 0
            ...
            validation_checks["dependencies"] = len([e for e in dependency_errors if e.severity == ErrorSeverity.ERROR]) == 0
        ...
        logger.info(f"Pre-flight validation completed in {duration:.2f}s: can_proceed={can_proceed}, errors={len(errors)}, warnings={len(warnings)}")
