...
class GroupScanProgressTracker:
        ...
        try:
            ...
            if scan_id:
                ...
                """
                    ...
                    {"session_id": session_id, "host_id": host_id, "scan_id": scan_id},
                )
    ...
    async def _update_session_progress(self, session_id: str):
        ...
        try:
            ...
            progress_percentage = (
                ((completed + failed + cancelled) / total_hosts) * 100 if total_hosts > 0 else 0
            )
            ...
            logger.debug(
                f"Updated session {session_id} progress: {progress_percentage:.1f}% complete"
            )
    ...
    async def calculate_progress(self, session_id: str) -> GroupScanProgress:
        ...
        try:
            ...
            progress_percentage = (
                ((hosts_completed + hosts_failed) / session_result.total_hosts) * 100
                if session_result.total_hosts > 0
                else 0
            )
    ...
    async def estimate_completion(self, session_id: str) -> Optional[datetime]:
        ...
        try:
            ...
            if (
                progress.average_scan_duration
                and progress.hosts_scanning + progress.hosts_pending > 0
            ):
...
class GroupScanService:
    ...
    async def initiate_group_scan(
        self, group_id: int, user_id: int, scan_config: Optional[GroupScanConfig] = None
    ) -> GroupScanSession:
        ...
        try:
            ...
            if scan_config.content_id:
                ...
                if scan_config.profile_id and content_result.profiles:
                    ...
                    if scan_config.profile_id not in profile_ids:
                        raise ValueError(
                            f"Profile {scan_config.profile_id} not found in SCAP content"
                        )
            else:
                # Use default SCAP content
                default_content = self.db.execute(
                    text(
                        """
                    SELECT id, name, file_path, profiles FROM scap_content 
            ...
            session_data["estimated_completion"] = datetime.utcnow() + timedelta(
            ...
            logger.info(
                f"Created group scan session {session_id} for group {group_id} with {len(hosts)} hosts"
            )
            return session
    ...
    async def start_group_scan_execution(self, session_id: str) -> bool:
        ...
        try:
            ...
            asyncio.create_task(
                self._execute_staggered_scans(session_id, hosts, content_result, scan_config)
            )

            logger.info(
                f"Started group scan execution for session {session_id} with {len(hosts)} hosts"
            )
            return True
    ...
    async def _execute_staggered_scans(
        self, session_id: str, hosts: List[Any], content_result: Any, scan_config: GroupScanConfig
    ):
    ...
    async def _create_individual_scan(
        self, session_id: str, host: Any, content_result: Any, scan_config: GroupScanConfig
    ) -> Optional[str]:
            ...
            """
                ),
                {
                    ...
                    "scan_options": json.dumps(
                        {"group_scan": True, "session_id": session_id, **scan_config.scan_options}
                    ),
    ...
    async def _execute_host_scan(
        ...
        try:
            logger.info(
                f"Starting host scan {scan_id} for {host.host_name} in session {session_id}"
            )
            ...
            await self.progress_tracker.update_host_status(
                session_id, host.host_id, HostScanStatus.FAILED.value, scan_id, error_message=str(e)
            )
