...
class HostMonitor:
    ...
    async def check_ssh_connectivity(
        ...
        try:
            ...
            if key_path:
                ...
                try:
                    ...
                    if not validation_result.is_valid:
                        logger.error(
                            f"Invalid SSH key for {ip_address}: {validation_result.error_message}"
                        )
                    ...
                    if validation_result.warnings:
                        logger.warning(
                            f"SSH key warnings for {ip_address}: {'; '.join(validation_result.warnings)}"
                        )
            ...
            logger.warning(
                f"SSH authentication failed for {ip_address} - check credentials in Settings"
            )
    ...
    async def get_effective_ssh_credentials(self, host_data: Dict, db) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Failed to resolve credentials for host monitoring {host_data.get('hostname')}: {e}"
            )
            return None
            ...
            if ssh_credentials:
                ...
                if not is_valid:
                    ...
                    check_results["credential_details"] = (
                        f"Using {source} credentials (user: {username}, method: {auth_method})"
                    )

                    logger.info(
                        f"Checking SSH connectivity for {hostname} using {source} credentials (user: {username}, method: {auth_method})"
                    )
                    ...
                    if ssh_success:
                        ...
                        logger.info(
                            f"SSH authentication successful for {hostname} using {source} credentials"
                        )
                    else:
                        check_results[
                            "credential_details"
                        ...
                        logger.warning(
                            f"SSH authentication failed for {hostname} using {source} credentials: {ssh_error}"
                        )
