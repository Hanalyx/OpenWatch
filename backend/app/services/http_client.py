    ...
    async def _execute_request(self, method: str, url: str, **kwargs) -> httpx.Response:
        ...
        if not self.circuit_breaker.can_execute():
            ...
            raise httpx.ConnectError(
                "Circuit breaker is open - service unavailable", request=httpx.Request(method, url)
            )
...
class WebhookHttpClient(HttpClient):
    ...
    def __init__(self):
        ...
        circuit_breaker_config = CircuitBreakerConfig(
            failure_threshold=5, recovery_timeout=300, success_threshold=2  # 5 minutes for webhooks
        )
