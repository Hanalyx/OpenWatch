...
class IntegrationMetricsCollector:
    ...
    def get_metrics_summary(self, operation: Optional[str] = None, 
                           hours: int = 1) -> Dict[str, MetricsSummary]:
    ...
    def get_current_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            stats["top_operations"] = dict(sorted(
    ...
    def export_metrics(self, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            for operation, summary in summaries.items():
                ...
                lines.append(f"integration_{safe_op}_duration_seconds_sum {summary.average_duration * summary.total_requests}")
...
def record_api_call(operation: str, success: bool, duration: float, 
                   service: str, error: str = None):
...
def record_remediation_job(job_id: str, status: str, duration: float, 
                          rules_count: int, success_count: int):
...
time_webhook_delivery = lambda target: metrics_collector.time_operation("webhook_delivery", {"target": target})
time_api_call = lambda operation, service: metrics_collector.time_operation(f"api_call_{operation}", {"service": service})
time_remediation = lambda job_id: metrics_collector.time_operation("remediation_execution", {"job_id": job_id})