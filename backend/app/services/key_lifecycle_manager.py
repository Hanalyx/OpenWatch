...
class RSAKeyMetadata:
    ...
    def to_dict(self) -> Dict:
        ...
        for field in ['created_at', 'activated_at', 'deprecated_at', 'expires_at', 'last_used']:
            if data[field]:
...
class RSAKeyLifecycleManager:
    ...
    def store_key_pair(self, 
                      private_key: rsa.RSAPrivateKey, 
                      public_key: rsa.RSAPublicKey,
                      key_id: str,
                      metadata: RSAKeyMetadata) -> Tuple[Path, Path]:
        """
        ...
        try:
            for key_dir in self.key_storage_path.iterdir():
                if key_dir.is_dir():
                    metadata = self.load_key_metadata(key_dir.name)
                    if (metadata and 
                        metadata.status == KeyStatus.ACTIVE and
                        metadata.expires_at and
                        metadata.expires_at <= warning_threshold):
        ...
        try:
            for key_dir in self.key_storage_path.iterdir():
                if key_dir.is_dir():
                    metadata = self.load_key_metadata(key_dir.name)
                    if (metadata and 
                        metadata.status in [KeyStatus.DEPRECATED, KeyStatus.REVOKED] and
