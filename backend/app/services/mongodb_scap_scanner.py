...
import logging
import subprocess  # nosec B404 (secure usage with argument lists)
import tempfile
import xml.etree.ElementTree as ET  # nosec B405 (parsing trusted local SCAP content)
...
from .remote_scap_executor import RemoteSCAPExecutionError, RemoteSCAPExecutor, ScanType
...
class MongoDBSCAPScanner(SCAPScanner):
    ...
    async def _merge_inherited_rule(
        self, child_rule: ComplianceRule, parent_data: Dict, platform: str
    ) -> ComplianceRule:
    ...
    async def generate_mongodb_scan_profile(
        self, rules: List[ComplianceRule], profile_name: str, platform: str
    ...
    def _generate_oval_definitions(
        ...
        try:
            ...
            for oval_info in oval_definitions_found:
                try:
                    # Parse the OVAL file (trusted local content only)
                    tree = ET.parse(oval_info["oval_path"])  # nosec B314
    ...
    def _get_platform_oval_filename(
        self, rule: ComplianceRule, target_platform: str
    ) -> Optional[str]:
        """
        Get platform-specific OVAL filename from Option B schema.
    ...
    def _generate_xccdf_profile_xml(
        ...
        profile_name: str,
        platform: str,
        rule_to_oval_id_map: Dict[str, str] = None,
    ) -> str:
        """
        ...
        if rules:
            sample_rule = rules[0]
            logger.info(
                f"Sample rule platform_implementations keys: {list(sample_rule.platform_implementations.keys()) if hasattr(sample_rule, 'platform_implementations') and sample_rule.platform_implementations else 'None'}"
            )
        ...
        for rule in rules:
            ...
            if actual_oval_id:
                ...
                logger.debug(
                    f"Rule {rule.rule_id} has no OVAL definition for target platform (skipped)"
                )
    ...
    async def scan_with_mongodb_rules(
        self,
        host_id: str,
        hostname: str,
        platform: str,
    ...
    async def _execute_mongodb_scan(
        self,
        scan_id: str,
        hostname: str,
        profile_path: str,
        ...
        try:
            ...
            if connection_params:
                ...
                try:
                    ...
                    logger.info(
                        f"Resolved credentials: username={credential_data.username}, "
                ...
                logger.info("Executing local MongoDB scan")

                # Security: Build command as list to prevent command injection
                # Using argument list (not shell=True) ensures all arguments are treated as literals
                # Per OWASP Command Injection Prevention: argument list approach
                cmd = [
                    "oscap",
                    "xccdf",
                    "eval",
                    "--profile",
                    profile_id,  # User-provided but safely passed as separate argument
                    "--results",
                    str(result_file),
                    "--report",
                    str(result_file).replace(".xml", ".html"),
                    profile_path,  # File path safely passed as separate argument
                ]

                logger.info(f"Executing local command: {' '.join(cmd)}")

                # Security: subprocess.run() with list argument (NO shell=True)
                # This prevents command injection by avoiding shell interpretation
                result = subprocess.run(  # nosec B603
                    cmd,  # List of arguments (secure)
                    capture_output=True,
                    text=True,
    ...
    async def _enrich_scan_results(self, scan_result: Dict, rules: List[ComplianceRule]) -> Dict:
        ...
        try:
            ...
            # Read and parse results (for future processing)
            with open(result_file, "r") as f:
                _results_xml = f.read()  # noqa: F841 - Reserved for future XML processing
