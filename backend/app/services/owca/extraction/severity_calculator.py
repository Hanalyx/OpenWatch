"""
OWCA Extraction Layer - Severity-Based Risk Calculator
...
class SeverityRiskResult(BaseModel):
    """
    Pydantic model for severity-based risk score calculation results.
...
class SeverityCalculator:
    """
    Calculator for severity-weighted risk scores from compliance scans.

    Part of OWCA Extraction Layer (Layer 0).

    This calculator provides standardized risk scoring based on finding severity
    ...
    Methods:
        ...
        get_severity_contribution: Calculate contribution of specific severity
    """

    def __init__(self):
        """Initialize severity calculator with industry-standard weights"""
        self.weights = SEVERITY_WEIGHTS
        logger.debug("SeverityCalculator initialized with weights: %s", self.weights)
    ...
    def calculate_risk_score(
        ...
        audit_logger.info(
            f"Severity risk score calculated: {result.risk_score} ({result.risk_level})",
            extra={
                "event_type": "SEVERITY_RISK_CALCULATED",
                "user_id": user_id,
                "scan_id": scan_id,
        ...
        logger.info(
            f"Severity risk score: {result.risk_score} ({result.risk_level}) - "
    ...
    def calculate_from_failed_rules(
        ...
        Returns:
            SeverityRiskResult with calculated score and breakdown

        Example:
            >>> calculator = SeverityCalculator()
            >>> failed = {'critical': 1, 'high': 5, 'medium': 15, 'low': 30}
            >>> result = calculator.calculate_from_failed_rules(failed)
    ...
    def get_severity_contribution(self, severity: str, count: int) -> float:
        ...
        Example:
            >>> calculator = SeverityCalculator()
            >>> contribution = calculator.get_severity_contribution('critical', 3)
