...
class PlatformCapabilityService:
    ...
    async def _detect_capability_type(
        ...
        if platform not in self.detection_commands:
            return {'detected': False, 'reason': f'Unsupported platform: {platform.value}'}
        
        commands = self.detection_commands[platform]
        if capability_type not in commands:
            return {'detected': False, 'reason': f'No detection method for {capability_type.value}'}
    ...
    async def get_platform_baseline(
        ...
        baselines = {
            'rhel': {
                '8': {
                    'expected_packages': [
                        ...
                        'rpm', 'yum', 'dnf', 'firewalld', 'openssh-server'
                    ],
                    'expected_services': [
                        'systemd', 'dbus', 'NetworkManager', 'sshd'
                    ],
                    'security_features': [
                        'selinux', 'firewall', 'crypto_policies'
                    ]
                },
                '9': {
                    'expected_packages': [
                        'systemd', 'kernel', 'glibc', 'bash', 'coreutils',
                        'rpm', 'dnf', 'firewalld', 'openssh-server'
                    ],
                    'expected_services': [
                        'systemd', 'dbus', 'NetworkManager', 'sshd'
                    ],
            ...
            'ubuntu': {
                '20.04': {
                    'expected_packages': [
                        'systemd', 'linux-image', 'libc6', 'bash', 'coreutils',
                        'dpkg', 'apt', 'ufw', 'openssh-server'
                    ],
                    'expected_services': [
                        'systemd', 'dbus', 'NetworkManager', 'sshd'
                    ],
                    'security_features': [
                        'apparmor', 'ufw', 'unattended_upgrades'
                    ]
                },
                '22.04': {
                    'expected_packages': [
