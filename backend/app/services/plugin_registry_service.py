...
from ..models.plugin_models import (
    ...
    PluginAssociation, PluginExecutionRequest, PluginExecutionResult
)
...
class PluginRegistryService:
    ...
    async def get_plugin_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Recent activity
            recent_imports = await InstalledPlugin.find().sort(
                -InstalledPlugin.imported_at
            ).limit(5).to_list()
    ...
    async def cleanup_unused_plugins(
        ...
        try:
            ...
            async for plugin in InstalledPlugin.find():
                should_cleanup = (
                    len(plugin.applied_to_rules) == 0 and  # Not applied to any rules
                    plugin.usage_count == 0 and  # Never used
                    plugin.imported_at < cutoff_date  # Old enough
                )
    ...
    async def _validate_plugin_registration(self, plugin: InstalledPlugin) -> Dict[str, Any]:
        ...
        if not plugin.executors:
            return {
                'valid': False,
                'error': 'Plugin has no executors defined'
            }
