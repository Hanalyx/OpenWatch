...
class PluginExecutionService:
    ...
    async def _create_execution_environment(
        ...
        context = {
            ...
            "host_info": {"host_id": request.host_id, "platform": request.platform},
            "config": {
    ...
    async def _select_executor(self, plugin: InstalledPlugin, platform: str) -> Optional[Dict[str, Any]]:
        ...
        return {"name": name, "executor": executor, "type": executor.type.value}
    ...
    async def _execute_python_plugin(
        ...
        command = [
            ...
            str(entry_point),
            "--context-file",
            str(execution_env["context_file"]),
        ]
