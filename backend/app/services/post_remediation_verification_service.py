...
from .bulk_remediation_service import BulkRemediationService, BulkRemediationResult
...
class PostRemediationVerificationService:
    ...
    async def cancel_verification(self, verification_id: str) -> bool:
        ...
        if verification.status in [VerificationStatus.COMPLETED, VerificationStatus.FAILED, VerificationStatus.CANCELLED]:
    ...
    async def list_continuous_monitors(
        ...
        return await ContinuousComplianceMonitor.find(query).sort(
            -ContinuousComplianceMonitor.created_at
        ).limit(limit).to_list()
    ...
    async def _execute_verification_after_delay(self, verification: VerificationJob):
        ...
        if verification.delay_before_verification_seconds > 0:
            logger.info(f"Waiting {verification.delay_before_verification_seconds}s before verification: {verification.verification_id}")
        ...
        try:
            ...
            for host_id in verification.target_hosts:
                try:
                    ...
                    if host_result.verification_outcome in [VerificationOutcome.SUCCESS, VerificationOutcome.PARTIAL_SUCCESS]:
            ...
            logger.info(f"Verification completed: {verification.verification_id} - {verification.hosts_with_improvements}/{verification.hosts_verified} hosts improved")
    ...
    async def _execute_continuous_monitor(self, monitor: ContinuousComplianceMonitor):
        ...
        for host_result in verification.host_results:
            if host_result.verification_outcome in [VerificationOutcome.FAILED, VerificationOutcome.PARTIAL_SUCCESS]:
                drift_detected = True
                break
        ...
        if drift_detected:
            ...
            if (monitor.auto_remediate and 
                monitor.auto_remediations < monitor.max_auto_remediations):
