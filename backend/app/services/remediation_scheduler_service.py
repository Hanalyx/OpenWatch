...
class RemediationSchedulerService:
    ...
    async def get_schedule_executions(
        ...
        return await ScheduleExecution.find(
            ScheduleExecution.schedule_id == schedule_id
        ).sort(-ScheduleExecution.started_at).limit(limit).to_list()
    ...
    async def get_upcoming_schedules(self, hours_ahead: int = 24) -> List[RemediationSchedule]:
        ...
        return await RemediationSchedule.find({
            "status": ScheduleStatus.ACTIVE,
            "next_execution_at": {"$lte": cutoff, "$gte": datetime.utcnow()}
        }).sort(RemediationSchedule.next_execution_at).to_list()
                ...
                if schedule.requires_approval:
                    if not schedule.approved_at:
                        ...
                        if schedule.approval_required_by and now > schedule.approval_required_by:
                            logger.warning(f"Schedule {schedule.schedule_id} missed approval deadline, skipping execution")
