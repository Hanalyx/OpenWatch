...
class RemediationJob(BaseModel):
    ...
    openwatch_context: Dict[str, Any] = Field(
        default_factory=dict,
        description="Context from OpenWatch scan results"
    )
...
class OpenWatchRemediationSystemAdapter:
    ...
    def _map_capabilities_to_plugin_capabilities(
        self, 
        capabilities: List[RemediationSystemCapability]
    ) -> List:
        ...
        mapping = {
            ...
            RemediationSystemCapability.SECURITY_HARDENING: [PluginCapability.ANSIBLE, PluginCapability.SHELL],
            RemediationSystemCapability.COMPLIANCE_REMEDIATION: [PluginCapability.API],
            RemediationSystemCapability.CUSTOM_SCRIPTING: [PluginCapability.SHELL, PluginCapability.PYTHON],
        }
...
'''
    ...
    def _convert_to_openwatch_result(
        ...
        return PluginExecutionResult(
            ...
            error=None if job_result.status == RemediationExecutionStatus.SUCCESS else f"Job failed with status: {job_result.status}",
            ...
            } if any(r.rollback_available for r in job_result.rule_results) else None
        )
