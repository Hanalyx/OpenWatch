        ...
        for pattern_key, failures in failure_patterns.items():
            if len(failures) >= 2:  # Systematic failure (affects multiple hosts/scans)
                ...
                gap = ComplianceGap(
                    ...
                    description=f"Rule {rule_id} fails systematically across {len(failures)} hosts ({failure_rate:.1%} failure rate)",  # noqa: E501
    ...
    async def _generate_recommendations(self, aggregated_results: AggregatedResults) -> None:
        ...
        for framework_id, metrics in aggregated_results.framework_metrics.items():
            if metrics.compliance_percentage < 70:
                priority_recommendations.append(
                    f"URGENT: {framework_id} compliance at {metrics.compliance_percentage:.1f}% - below acceptable threshold"  # noqa: E501
                )
            elif metrics.compliance_percentage >= 95:
                strategic_recommendations.append(
                    f"EXCELLENCE: {framework_id} compliance at {metrics.compliance_percentage:.1f}% - consider advanced security measures"  # noqa: E501
                )
        ...
        if total_exceeds > 0:
            strategic_recommendations.append(
                f"OPPORTUNITY: {total_exceeds} rules exceed baseline requirements - leverage for enhanced compliance reporting"  # noqa: E501
            )

        # Performance recommendations
        if aggregated_results.performance_metrics.get("success_rate", 100) < 95:
            priority_recommendations.append(
                f"RELIABILITY: Execution success rate at {aggregated_results.performance_metrics.get('success_rate', 0):.1f}% - investigate infrastructure issues"  # noqa: E501
            )

        # Platform diversity recommendations
        if len(aggregated_results.platform_distribution) > 1:
            strategic_recommendations.append(
                "STANDARDIZATION: Multiple platforms detected - consider standardization for consistent compliance"
            )
