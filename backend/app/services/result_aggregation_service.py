...
class ResultAggregationService:
    ...
    async def _aggregate_organization_level(
        ...
        aggregated_results.execution_statistics = {
            ...
            'average_execution_time': statistics.mean([e.execution_time for e in all_executions]) if all_executions else 0.0,
            'median_execution_time': statistics.median([e.execution_time for e in all_executions]) if all_executions else 0.0
        }
        
        # Calculate performance metrics
        if all_executions:
            aggregated_results.performance_metrics = {
                'rules_per_second': len(all_executions) / sum(sr.total_execution_time for sr in scan_results) if sum(sr.total_execution_time for sr in scan_results) > 0 else 0.0,
                ...
                'compliance_rate': len([e for e in all_executions if e.compliance_status in [ComplianceStatus.COMPLIANT, ComplianceStatus.EXCEEDS]]) / len(all_executions) * 100
            }
    ...
    async def _compare_frameworks(
        ...
        if compliance_a and compliance_b:
            ...
            correlation = statistics.correlation(
                compliance_a[:min_length], 
                compliance_b[:min_length]
            ) if min_length > 1 else 0.0
        else:
            correlation = 0.0
