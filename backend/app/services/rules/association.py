...
Handles intelligent mapping between OpenWatch compliance rules and available
remediation plugins. Provides semantic matching, confidence scoring, and
plugin recommendation capabilities.
...
Example:
    ...
    ... )
    >>>
    >>> service = RuleAssociationService()
    >>> mappings = await service.get_mappings_for_rule(rule_id, platform="rhel8")
"""
...
class RulePluginMapping(Document):
    ...
    is_validated: bool = Field(
        default=False,
        description="Mapping has been validated through execution",
    )
...
class RuleAssociationService:
    ...
    async def create_mapping(
        ...
        logger.info(
            f"Created rule mapping: {openwatch_rule_id} -> {plugin_id} "
            f"({platform}, {confidence.value})"
        )
        return mapping
    ...
    async def recommend_plugins_for_rules(
        ...
        Args:
            ...
            platform: Target platform
            framework: Compliance framework
            prefer_existing_mappings: Use existing mappings if available

        Returns:
            Dictionary mapping rule_id to recommendations
        """
        recommendations = {}
            ...
            rules.append(
                {
                    ...
                    "description": (
                        plugin.description or f"Generic remediation using {plugin.name}"
                    ),
                }
            )
