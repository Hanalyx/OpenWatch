...
class SCAPScanner:
    ...
    def test_ssh_connection(
        ...
        try:
            ...
            if auth_method == "password":
                ssh.connect(hostname, port=port, username=username, password=credential, timeout=10)
                ...
                try:
                    ...
                    if not validation_result.is_valid:
                        logger.error(
                            f"Invalid SSH key for {hostname}: {validation_result.error_message}"
                        )
                    ...
                    if validation_result.warnings:
                        logger.warning(
                            f"SSH key warnings for {hostname}: {'; '.join(validation_result.warnings)}"
                        )
    ...
    def _is_configuration_line(self, line: str) -> bool:
        ...
        config_patterns = ["=", ":", "yes", "no", "true", "false", "enabled", "disabled"]
        ...
        try:
            ...
            for line in lines:
                if self._is_command_line(line):
                    ...
                    commands.append(
                        {"command": line, "type": "config", "description": "Configuration setting"}
                    )
    ...
    def _extract_framework_specific_remediation(
        ...
        try:
            ...
            stig_elements = [".//stig:fix_text", ".//stig:fixtext", ".//fixtext", ".//fix_text"]
    ...
    def _execute_remote_scan_with_paramiko(
        ...
        try:
            ...
            if auth_method == "password":
                ssh.connect(hostname, port=port, username=username, password=credential, timeout=30)
            elif auth_method in ["ssh-key", "ssh_key"]:
                # Handle SSH key authentication using new utility
                try:
                    # Validate SSH key first
