"""
XCCDF Score Extraction Service
...
This service implements Phase 1 of the XCCDF Scoring Implementation Plan:
...
Example:
    >>> from backend.app.services.scoring.xccdf_score_extractor import XCCDFScoreExtractor
    >>> extractor = XCCDFScoreExtractor()
    >>> result = extractor.extract_native_score("/app/data/results/scan_123_xccdf.xml")
...
class XCCDFScoreExtractor:
    """
    Service for extracting XCCDF native scores with comprehensive security controls.
    ...
    def __init__(self):
        """Initialize XCCDF score extractor with secure parser."""
    ...
    def extract_native_score(self, result_file: str, user_id: Optional[str] = None) -> XCCDFScoreResult:
        ...
        Example:
            >>> extractor = XCCDFScoreExtractor()
            >>> result = extractor.extract_native_score("/app/data/results/scan_123.xml")
            >>> if result.found:
            ...     print(f"Score: {result.xccdf_score}/{result.xccdf_score_max}")
        """
        try:
    ...
    def _is_safe_path(self, file_path: str) -> bool:
        ...
        Example:
            >>> extractor._is_safe_path("/app/data/results/scan.xml")  # Safe
            True
            >>> extractor._is_safe_path("../../../etc/passwd")  # Unsafe
            False
        """
