...
class SecureAutomatedFix:
    ...
    def _determine_security_level(self) -> CommandSecurityLevel:
        ...
        elif any(dangerous in (self.command or "").lower() 
    ...
    def _requires_approval(self) -> bool:
        ...
        return (self.requires_sudo or 
                self.security_level in [CommandSecurityLevel.PRIVILEGED, CommandSecurityLevel.CRITICAL] or
                not self.is_safe)
...
class FixExecutionAudit:
    ...
    async def log_fix_request(self, fix_id: str, requested_by: str, 
                            target_host: str, justification: str):
    ...
    async def log_fix_approval(self, request_id: str, approved_by: str, 
                             approval_reason: str):
    ...
    async def log_fix_execution(self, request_id: str, execution_result: ExecutionRequest):
        ...
        entry = {
            ...
            "execution_duration": (execution_result.completed_at - execution_result.executed_at).total_seconds() if execution_result.completed_at and execution_result.executed_at else None,
    ...
    async def log_fix_rollback(self, request_id: str, rollback_by: str, 
        ...
        try:
            async with get_async_db() as session:
                ...
                await session.execute(audit_sql, {
                    ...
                    "old_values": None,
                    "new_values": json.dumps({k: v for k, v in entry.items() if k not in ["event_type", "timestamp"]}),
                    "ip_address": "system",
...
class SecureAutomatedFixExecutor:
    ...
    async def evaluate_fix_options(self, legacy_fixes: List[AutomatedFix], 
        ...
        for legacy_fix in legacy_fixes:
            ...
            if secure_fix.secure_command_id:
                ...
                secure_options.append({
                    ...
                    "description": f"⚠️ SECURITY BLOCKED: {legacy_fix.description}",
    ...
    async def request_fix_execution(self, fix_id: str, secure_command_id: str,
                                  parameters: Dict[str, Any], target_host: str,
    ...
    async def approve_fix_request(self, request_id: str, approved_by: str, 
    ...
    async def execute_approved_fix(self, request_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            response = {
                ...
                "execution_time": (result.completed_at - result.executed_at).total_seconds() if result.completed_at and result.executed_at else None,
    ...
    async def rollback_fix(self, request_id: str, rollback_by: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": False,
                "message": f"Failed to rollback fix: {str(e)}"
            }
    ...
    async def cleanup_old_requests(self, max_age_days: int = 30):
        ...
        expired_requests = [
            req_id for req_id, data in self.pending_approvals.items()
