...
class SemanticSCAPEngine:
    ...
    async def process_scan_with_intelligence(
        ...
        try:
            ...
            logger.info(f"Semantic analysis complete for scan {scan_id}: "
            ...
            logger.error(f"Error in semantic SCAP processing for scan {scan_id}: {e}", exc_info=True)
    ...
    async def _extract_semantic_understanding(
        ...
        rule_details_lookup = {
            detail.get("rule_id"): detail for detail in rule_details
        }
    ...
    async def _map_to_universal_frameworks(
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.aegis_base_url}/api/v1/frameworks",
                    timeout=5.0
                )
