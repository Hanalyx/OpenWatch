...
def detect_key_type(key_content: str) -> Optional[SSHKeyType]:
    ...
    if isinstance(key_content, (bytes, memoryview)):
        try:
            key_content = key_content.decode('utf-8') if isinstance(key_content, bytes) else key_content.tobytes().decode('utf-8')
...
def assess_key_security(key_type: SSHKeyType, key_size: Optional[int]) -> Tuple[SSHKeySecurityLevel, list, list]:
    ...
    if key_type == SSHKeyType.ED25519:
        ...
        if key_size is None:
            ...
            return SSHKeySecurityLevel.ACCEPTABLE, warnings, ["Verify key size meets security requirements"]
        
        if key_size < 2048:
            return SSHKeySecurityLevel.REJECTED, ["RSA key size too small"], ["Use RSA-4096 or Ed25519 keys"]
        elif key_size < 3072:
            return SSHKeySecurityLevel.DEPRECATED, ["RSA-2048 keys are deprecated"], ["Upgrade to RSA-4096 or Ed25519"]
        elif key_size < 4096:
            warnings.append("RSA-3072 keys are acceptable but RSA-4096 is recommended")
            return SSHKeySecurityLevel.ACCEPTABLE, warnings, ["Consider upgrading to RSA-4096 or Ed25519"]
        else:
            return SSHKeySecurityLevel.SECURE, warnings, recommendations
        ...
        if key_size and key_size >= 256:
            ...
            return SSHKeySecurityLevel.ACCEPTABLE, warnings, ["Verify ECDSA curve meets requirements"]
    
    elif key_type == SSHKeyType.DSA:
        return SSHKeySecurityLevel.REJECTED, ["DSA keys are deprecated and insecure"], ["Use Ed25519 or RSA-4096 keys"]
...
def validate_ssh_key(key_content: str, passphrase: Optional[str] = None) -> SSHKeyValidationResult:
    ...
    if not key_content or not key_content.strip():
        return SSHKeyValidationResult(
            is_valid=False,
            error_message="SSH key content is empty"
        )
    ...
    try:
        ...
        return SSHKeyValidationResult(
            is_valid=False,
            key_type=key_type,
            error_message=f"Failed to parse SSH key: {e}"
        )
