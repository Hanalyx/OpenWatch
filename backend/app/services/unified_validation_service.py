...
class UnifiedValidationService:
    ...
    async def _test_network_connectivity(self, hostname: str, port: int) -> Dict:
        ...
        try:
            ...
            if result == 0:
                ...
                return {"success": False, "error": f"Cannot connect to {hostname}:{port}"}
        except Exception as e:
            return {"success": False, "error": f"Network connectivity test failed: {str(e)}"}
    ...
    async def _test_openscap_dependencies(
        ...
        try:
            ...
            return {"success": False, "error": f"OpenSCAP dependency check failed: {str(e)}"}
