...
def scheduled_group_scan(self, group_id: int, config: Dict[str, Any]):
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            group = db.query(HostGroup).filter(
                HostGroup.id == group_id,
                HostGroup.auto_scan_enabled == True
            ).first()
...
def execute_compliance_scan_async(self, session_id: str, group_id: int, 
                                hosts: List[Dict], config: Dict[str, Any]):
    """
    ...
    try:
        with get_db_session() as db:
            ...
            if config.get("email_notifications"):
                send_compliance_notification.delay(session_id, group_id, {
                    "successful_scans": successful_scans,
                    "failed_scans": failed_scans,
                    "total_hosts": len(hosts)
                })
            
            print(f"Compliance scan completed for group {group_id}: {successful_scans} successful, {failed_scans} failed")
