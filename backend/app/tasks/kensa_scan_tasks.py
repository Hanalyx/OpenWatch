...
def create_kensa_scan_record(
    ...
    scan_name = name or f"Kensa Scan - {hostname} - {start_time.strftime('%Y-%m-%d %H:%M')}"
    profile_id = f"kensa_{framework}" if framework else "kensa_cis"
    ...
    insert_builder = (
        ...
        .values(
            ...
            start_time,
            user_id,
            f'{{"scanner": "kensa", "framework": "{framework or "cis"}", "quick_scan": true}}',
            False,
            False,
        )
    )
