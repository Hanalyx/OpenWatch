...
def execute_scan_task(
    scan_id: str, host_data: Dict, content_path: str, profile_id: str, scan_options: Dict
):
    """
    ...
    try:
        ...
        if group_scan_session_id and scan_options.get("group_scan"):
            try:
                ...
                progress_tracker.update_host_status(
                    group_scan_session_id, host_data.get("id", "unknown"), "scanning", scan_id
                )
        ...
        if demo_mode:
            ...
            result_data = {
                ...
                "findings": [
                    ...
                    },
                    {
                        ...
                        "severity": "medium",
                        "status": "pass",
                        "description": "Security control passed",
                    },
                ],
            }
            ...
            """
                ...
                {"scan_id": scan_id, "completed_at": datetime.utcnow()},
            )
            db.commit()
            ...
            try:
                ...
                asyncio.create_task(send_scan_completed_webhook(scan_id, webhook_data))
                logger.debug(f"Webhook notification queued for demo scan: {scan_id}")
            except Exception as webhook_error:
                logger.error(
                    f"Failed to send demo completion webhook for scan {scan_id}: {webhook_error}"
                )
        ...
        if host_data["hostname"] != "localhost":
            ...
            if not ssh_test["success"]:
                ...
                _update_scan_error(
                    db, scan_id, f"SSH connection failed: {ssh_test['message']}", ssh_error
                )
                return
...
def _update_scan_error(
    ...
    try:
        ...
        """
            ...
            {"scan_id": scan_id, "completed_at": datetime.utcnow(), "error_message": error_message},
        )
        db.commit()
        ...
        if scan_data:
            try:
                ...
                asyncio.create_task(send_scan_failed_webhook(scan_id, webhook_data, error_message))
...
try:
    ...
    def execute_scan_celery_task(
        self, scan_id: str, host_data: Dict, content_path: str, profile_id: str, scan_options: Dict
    ):
