...
| 2026-04-29 | Policies are Ed25519-signed; admin keys embedded in binary | Same primitive as license signing and audit chain — one crypto surface. Filesystem permissions are insufficient (most attackers who can write the file can also run openwatch). Unsigned files load only with `OPENWATCH_DEV_MODE=true`. |
