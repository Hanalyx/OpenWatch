...
| 3 — Compliance tab lens | **DONE** | GET /hosts/{id}/compliance (+/frameworks) with C-05 reconciliation; ComplianceTab.tsx lens UI. Live: lens switch recounts 68.1% all-rules -> 71.4% under stig_rhel8 (266 rules exactly) |
...
2. **Trigger posture** — the vision says "auto-scan, no manual buttons," but both prototypes show `Run scan`/`Re-scan`. Confirm we ship **both** (on-demand + adaptive), with on-demand as the dev/first-contact path.
...
4. **Scheduler config source-of-truth** (Phase 4 (a)) — signed schedules-policy file vs operator-editable systemconfig. Recommendation: systemconfig, matching every other Settings → Scanning section.
5. **Compliance state bands** (Phase 4 (b)) — prototype's 5 bands vs the backend's 4 + `unknown`. The chosen names become the config keys and the lens/table row labels.
