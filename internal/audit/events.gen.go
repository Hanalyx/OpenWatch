...
const (
	...
	// Authentication attempt failed, or a presented credential was refused. reason names why. Password login records wrong_password, unknown_user, account_disabled, mfa_required, mfa_invalid, mfa_required_during_login or password_changed_during_login. The binders record the session, access-token and account-state reasons. SSO records the sso_ reasons, and sso_account_disabled or sso_account_deleted when the local account may not sign in; the sign-in page stays generic. A service-account token whose owner is disabled or deleted, or that has no owner, records api_token_owner_disabled, api_token_owner_deleted or api_token_ownerless. account_state_unavailable, session_lookup_failed and role_lookup_unavailable mean the server could not tell, and the request answered 503, not 401. invalid_credentials, account_locked, mfa_failed and sso_failed are legacy values that nothing records now; older rows may carry them. username is the submitted name, clipped. remote_addr and user_agent are recorded by the binders, the user agent clipped.
...
var Metadata = map[Code]EventMeta{
	...
	AuthLoginFailure: {
		...
		Description: `Authentication attempt failed, or a presented credential was refused. reason names why. Password login records wrong_password, unknown_user, account_disabled, mfa_required, mfa_invalid, mfa_required_during_login or password_changed_during_login. The binders record the session, access-token and account-state reasons. SSO records the sso_ reasons, and sso_account_disabled or sso_account_deleted when the local account may not sign in; the sign-in page stays generic. A service-account token whose owner is disabled or deleted, or that has no owner, records api_token_owner_disabled, api_token_owner_deleted or api_token_ownerless. account_state_unavailable, session_lookup_failed and role_lookup_unavailable mean the server could not tell, and the request answered 503, not 401. invalid_credentials, account_locked, mfa_failed and sso_failed are legacy values that nothing records now; older rows may carry them. username is the submitted name, clipped. remote_addr and user_agent are recorded by the binders, the user agent clipped.`,
		ActorTypes:  []string{"user"},
		DetailKeys:  []string{"auth_method", "reason", "remote_addr", "user_agent", "username"},
	},
	AuthLogout: {
		Code:        AuthLogout,
		Category:    "auth",
