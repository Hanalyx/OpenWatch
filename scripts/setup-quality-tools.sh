...
echo -e "${BOLD}${BLUE}"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "OpenWatch Quality Tools Setup"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo -e "${NC}\n"
...
if ! command -v pipx &> /dev/null; then
    echo -e "${YELLOW}[WARNING] pipx not found - installing...${NC}"
    ...
    echo -e "${GREEN}[OK] pipx installed${NC}\n"
else
...
echo -e "${GREEN}[OK] Python tools installed via pipx${NC}\n"
...
if ! command -v shellcheck &> /dev/null; then
    ...
    echo -e "${GREEN}[OK] shellcheck installed${NC}\n"
else
    echo -e "${GREEN}[OK] shellcheck already installed${NC}\n"
fi
...
if [ -f "backend/requirements-dev.txt" ]; then
    ...
    echo -e "${GREEN}[OK] Backend dev dependencies installed${NC}\n"
fi
...
if command -v pre-commit &> /dev/null; then
    echo -e "${GREEN}[OK] pre-commit found${NC}"
    ...
    echo -e "${GREEN}[OK] Commit-msg hook installed${NC}\n"

    # Generate secrets baseline if not exists
    echo -e "${BLUE}→ Checking secrets baseline...${NC}"
    if [ ! -f ".secrets.baseline" ]; then
        echo -e "${YELLOW}[WARNING] Generating .secrets.baseline (this may take a moment)...${NC}"
        detect-secrets scan > .secrets.baseline 2>/dev/null || echo "{}" > .secrets.baseline
        echo -e "${GREEN}[OK] .secrets.baseline generated${NC}\n"
    else
        echo -e "${GREEN}[OK] .secrets.baseline already exists${NC}\n"
    fi
    ...
    echo -e "${GREEN}[OK] Pre-commit setup complete${NC}\n"
else
...
if [ -f "scripts/quality-check.sh" ]; then
    ...
    echo -e "${GREEN}[OK] Quality check script ready${NC}\n"
else
    echo -e "${YELLOW}[WARNING] Quality check script not found${NC}\n"
fi
...
for tool in black isort flake8 mypy bandit safety pytest pre-commit detect-secrets shellcheck radon vulture; do
    if command -v $tool &> /dev/null; then
        ...
        echo -e "${GREEN}  [OK] $tool${NC} - $VERSION"
    else
...
echo -e "${GREEN}[OK] All quality tools installed${NC}"
echo -e "${GREEN}[OK] Pre-commit hooks configured${NC}"
echo -e "${GREEN}[OK] Ready to develop!${NC}"
